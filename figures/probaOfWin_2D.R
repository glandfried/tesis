...
mtext(text=expression("Desempeño"~p[j]) ,side =2 ,line=1.5,cex=2)
mtext(text =expression("Desempeño"~p[i]) ,side =1 ,line=2,cex=2)
abline(v=1750,lty=3)
abline(h=1500,lty=3)
#lines(c(0,1)*(s_2+s_2-500),c(1,0)*(s_2+s_2-500))
lines(c(0,1)*(s_2+s_2-500),c(0,1)*(s_2+s_2-500),col=rgb(0,0,0,0.75))
text(s_2-250,s_1+250,expression("Gana j ("~p[i]<p[j]~")"),cex=2)
text(s_1+250,s_2-350,expression("Gana i ("~p[i]>p[j]~")"),cex=2)
mtext(expression(p[i]==p[j]), side=3, line=0, at=2500,cex=1.25)
...
axis(lwd=0,side=1, at=s_1,labels=expression(s[i]),las=0,cex.axis=1.5,line=-0.75)
axis(lwd=0,side=2, at=s_2,labels=expression(s[j]),las=0,cex.axis=1.5,line=-0.75)
