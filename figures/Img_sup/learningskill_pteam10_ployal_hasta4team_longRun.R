...
lst89 <- read.csv("data/learningskill_pteam10_ployalSuperior_hasta4team.csv", header =T)
rn.max<-500
...
lst01 <- read.csv("data/learningskill_pteam10_ployalInf_hasta4team.csv", header =T)
...
lst <- read.csv("data/learningskill_pteam10_hasta4team.csv", header =T)
lst <- lst[1:rn.max,]
nn <- lst[,"count"] 
...
points(lst.points, pch=4,lwd=1.33,cex=1.33)
...
par(xpd=TRUE)
legend(0,y.max,
       c("All players","Weak TOB ","Loyal subclass", "Casual subclass"),
       lty=c(1,3,3,3),pch=c(NA,4,0,8),
