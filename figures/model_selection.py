    ...
    for d in range(10):#d=0
        for i in range(N) :#i=2
            ...
            if N >= 10:
                ...
                prior_maxAposteriori_online[d,0] += np.log(np.squeeze(likelihood(w_map_prior, t_posteriori, Phi_posteriori , beta)))
            mean_square_error_MAP[d,0] += (1/N) * np.squeeze(((t_posteriori - Phi_posteriori.dot(w_map_prior))**2))
            
                    
        Phi =  polynomial_basis_function(X, np.array(range(d+1)) )
        prior_predictive_joint[d,0] = np.log(predictive(t, Phi, beta, alpha ))
        log_evidence_joint[d,0] = np.squeeze(log_evidence(t, Phi, beta, alpha))
...
plt.plot(X_grilla, y_true, '--', color="black")
plt.plot(X,t,'.', color='black')
plt.axvline(x=0.1, color="gray", alpha=0.3)
