...
normal_product_dep <- function(x1,x2,mu1=25,sigma1=25/3,sigma2=25/6){
  return(dnorm(x1,mu1,sigma1*1.2)*dnorm(x2,mu1,sigma1*0.8))
}
ds <- 0.1;s_grilla <- seq(a,b,by=ds);p_grilla <- s_grilla
...
mtext("Performance j", side=2, line=2,cex = 1.75)
mtext("Performance i", side=1, line=2,cex = 1.75)
...
axis(lwd=1,side=2,at=25, labels= expression(mu[j]),cex.axis=1.5,line=-0.66)
lines(c(0,50),c(50,0))
