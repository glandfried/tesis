...
\end{equation}
%
Esta es la distribución de probabilidad condicional de las hipótesis $Y_i$ dado el modelo grado $D$, $M_D$.
%
Las distribuciones de probabilidad sobre variables continuas se notan con $p$ minúscula.
%
...
\end{equation}
%
En la figura~\ref{fig:modelo_lineal} se especifican las dependencias del modelo causal lineal mediante la notación de redes bayesianas que representan a las hipótesis ocultas con círculos blancos, las hipótesis observadas con círculos grises, los puntos negros como constantes (o parámetros) del modelo y las dependencias causales entre hipótesis mediante flechas.
%
\begin{figure}[ht!]
    \centering
...
\end{equation*}
%
Estos enfoques ad-hoc, en vez de predecir el valor de la hipótesis aún no observada $y_N$ con la contribución de todas las hipótesis no observables $\bm{w}$ mediante la aplicación estricta de las reglas de la probabilidad, aproximan la predicción mediante la selección del vector de hipótesis elementales $\bm{w}$ que maximiza la función de costo ad-hoc.
%
A diferencia de la predicción realizada mediante la regla de la suma, las predicciones realizadas con un único vector $\bm{w}$ de hipótesis elementales tiene mayor tendencia a asignar a priori probabilidad cercana a cero a la hipótesis elemental $y_N$ que finalmente se hace observable.
%
...
%
\begin{equation*}
p(y_i|x_i,\beta) = \N(y_i \,| \, \text{sen}(x_i), \beta^2)
\end{equation*}
%
En la figura~\ref{fig:model_selection_OLS} se grafica la sinoidal mediante una l\'inea punteada, y las observaciones $y_0$ a $y_N$ mediante puntos negros.
%
...
%
En primer lugar, sin la posibilidad de aplicar las reglas de la probabilidad, ¿qué significa que un modelo sea mejor que otro?

%

Los modelos de grado 0 a 2 no tienen la flexibilidad suficiente para ``ajustar'' (o acercarse) a la sinoidal en el rango $x_i \in [-\pi, \pi]$.
%
A partir del grado 3 todos los modelos tienen la flexibilidad suficiente para ``ajustar'' (o acercarse) a la sinoidal en el rango $x_i \in [-\pi, \pi]$.
%
En la figura~\ref{fig:model_selection_OLS} graficamos el ajuste de todos los modelos destacando el modelo de grado 3 (linea roja) y el modelo de grado 9 (línea celeste).
%
A medida que los modelos son más complejos (a mayor grado), tienen mayor flexibilidad y por lo tanto el valor de m\'axima verosimilitud aumenta (figura~\ref{fig:model_selection_maxLikelihood}).
%
El problema es que el modelo de grado 9, a pesar de ser el que m\'as cerca pasa por los todos puntos observados, no es el que m\'as cerca pasa por la funci\'on objetivo oculta, y por lo tanto no ser\'a el modelo que mejor desempe\~no tenga sobre datos nuevos.
%
...
\begin{figure}[ht!] \centering
  ...
  En la figura~\ref{fig:model_selection_maxApriori_online} se muestra la probabilidad del modelo calculando cada una de las predicciones individuales a priori, usando como dato de entrenamiento en cada paso los datos ya observados.
   }
...
Hemos visto en la sección anterior que para evaluar modelos necesitamos la predicción a priori de los datos, $P(\text{Datos}|\text{Modelo}, I)$ (ecuaci\'on~\ref{eq:p<datos|modelo>}), la que se descompone como una secuencia de predicciones individuales de cada dato, con la informaci\'on de todos los datos anteriores.
%
Si aproximamos el producto de predicciones $P(\bm{y}|\bm{x}, \bm{w}, \beta, \sigma, M_D)$ mediante un procedimiento de entrenamiento en línea, en el que en cada paso se selecciona un nuevo vector $\bm{w}$ en base a los datos vistos hasta el momento para predecir el siguiente dato, el modelo de grado 3 obtiene probabilidad 1 (figura~\ref{fig:model_selection_maxApriori_online}).
%
Podría pensarse que el procedimiento de entrenamiento en línea resuelve el problema del sobreajuste debido a que penaliza los modelos que tienen mayor complejidad que la necesaria.
%
Sin embargo, hemos incurrido nuevamente en un sobreajuste debido a que hemos seleccionado un único modelo, que por ejemplo no será capaz de predecir correctamente datos por fuera del rango $[-\pi, \pi]$.
%
...
%
En la figura~\ref{model_selection_MAP_non-informative.pdf} nuevamente la sinoidal (línea punteada), los datos (puntos negros) y el ``ajuste'' de los modelos (graficando solo el valor máximo a posteriori de $y_i$ para cada uno de los modelos, sin su incertidumbre asociada).
%
Y en la figura~\ref{fig:model_selection_evidence} observamos la probabilidad a posteriori de los modelos.
...
%
Y por otro lado, los modelos con complejidad mayor a la necesaria, si bien tienen menor probabilidad que el modelo de grado 3, no son rechazados.
%
...
%
En este caso no se produce ninguno de los problemas de sobreajuste que emergen de los enfoques con criterios de selección de hipótesis ad-hoc.
%
Si empezamos a ver datos por fuera del rango $[-\pi, \pi]$, los modelos más complejos están todavía disponibles para predecir los datos que no puede predecir el modelo de grado 3.
...
Este balance surge naturalmente de las reglas de la probabilidad.
%
