...
%
Esta distribuci\'on de probabilidad se corresponde con la figura~\ref{fig:preferencia_maxima} y no casualmente es la distribuci\'on que maximiza la incertidumbre (o entrop\'ia) entre todas las opciones que incorporan la \'unica informaci\'on disponible: el regalo est\'a en alguna de las tres cajas.
%
Para derivar las distribuciones de probabilidad que maximizan la entrop\'ia dada las restricciones disponibles, se suele usar los multiplicadores de Lagrange.
%
...
%
Supongamos que se nos pide que elijamos una caja y alguien (un or\'aculo) que conoce la ubicaci\'on del regalo muestra el interior de una de las cajas que \textbf{no} contiene el regalo.
%
...
En la sección \emph{\nameref{sec:reglas_de_la_probabilidad}} definimos el conjunto de datos sobre el que se evalúan las hipótesis de investigación $H_i \in \mathcal{H}$ mediante la fórmula general del teorema de Bayes \eqref{eq:teorema_bayes} como un subconjunto arbitrario de hipótesis elementales, $D \subset \textbf{H}$, tal que $P(Datos)>0$
%
Recordemos que la investigación (o experimento) $\mathcal{H} = \{H_1, \dots, H_N\}$ consiste en determinar, para cada hipótesis $H_i \in \mathcal{H}$ cuál de todas sus hipótesis elementales $h_i \in H_i$ ocurre.
%
Debido a que las hipótesis elementales al interior de cada hipótesis son mutuamente excluyentes, el conjunto de Datos no pueden contener más de una hipótesis elemental por hipótesis de investigación.
%
\begin{equation*}
D \subseteq \textbf{H} \land P(D)> 0 \implies (\forall \, H_i \in \mathcal{H})(\text{cardinal}(DH_i) \leq 1)
\end{equation*}
%
Según esta definición puramente formal podemos definir arbitrariamente $\sum_{H_i \in \mathcal{H}} \text{cardinal}(H_i)^N$ conjuntos de datos distintos.
%
Sin embargo, en ciencias empíricas el concepto de \emph{conjunto de datos} las hipótesis de investigación se evalúan sobre un único conjunto de datos, que se determina en función de una dimensión no capturada por la definición formal.

% Parrafo

El matemático y epistemólogo de esta casa de estudios, Gregorio Klimovsky, llamó \emph{base emp\'irica} al conjunto de supuestos que son aceptados como verdad indubitable (al menos momentáneamente) al interior de una cierta comunidad~\cite{klimovsky1994-desventuras}.
%
En la terminología de la Teoría de la Probabilidad, la conclusión de una investigación empírica (o experimento empírico) es $P(\text{Hipótesis}|\text{Base empírica})$.
%
La base emp\'irica está compuesto por lo que llamamos evidencia formal que no se pone en duda (el sistemas de restricciones o modelos) y la evidencia empírica (lo que coloquialmente llamamos datos).
%
Hemos visto en más de un ejemplo que, en caso de que no exista un consenso sobre la evidencia formal (modelos o sistema de restricciones), ella pasan a formar parte de las hipótesis de investigación $\mathcal{H}$ y son evaluadas mediante la aplicación estricta de las reglas de la probabilidad.

% Parrafo

El conjunto de base empírica depende

Klimovsky llama \emph{base emp\'irica filos\'ofica} al conjunto de supuestos aceptados incluso
