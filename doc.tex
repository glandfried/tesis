...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption{Sensibilidad a condiciones iniciales en caos en el modelo de crecimiento poblacional: $\text{pop}_0 = 0.5$ (negro), $\text{pop}_0 = 0.499$ (rojo), en tanto que $r=3.99$ para los dos casos.}
    \label{fig:sensibilidad}
\end{figure}
%
Es decir, a pesar de que el sistema sea determinista y conozcamos el mecanismo causal que lo produce, no ser capaces de conocer las condiciones iniciales con precisi\'on \emph{infinitesimal} (debido a limitaciones en la representaci\'on computacional o por ruido en la medici\'on de datos) es suficiente para impedirnos predecir el comportamiento del sistema m\'as all\'a de un breve lapso de tiempo.
%
A esta incertidumbre, que proviene de nuestra ignorancia, se la conoce como \textbf{epist\'emica}.
...
%
La incertidumbre asociada a eventos aleatorios, que ocurrir\'ian sin raz\'on suficiente, se la conoce como \textbf{ontol\'ogica}

% Parrafo

Sea porque el mundo natural admite la posibilidad de que un efecto surja sin una raz\'on suficiente, o sea por nuestra incapacidad de conocer los modelos causales correctos o las condiciones iniciales con precisi\'on infinita, la incertidumbre nos impide describir el mundo mediante funciones deterministas del estilo $f(x)=y$, pues no tenemos un \'unico valor $y$ para mapear (por medio de la funci\'on $f$) cada valor de $x$, sino que cada uno de estos le corresponde un conjunto de valores posibles $Y$.
%
A este conjunto se lo conoce como \textbf{espacio de hip\'otesis}.
...
%
Vamos a suponer que la creencia total se representa con la unidad, en tanto que una funci\'on de probabilidad asigna una fracci\'on de la creencia total a cada posible hip\'otesis $h$. 
%
En la teor\'ia de la probabilidad, la regla de la suma garantiza el principio de integridad, esto es que no se produzcan p\'erdidas ni creaciones espont\'aneas de creencias cuando la distribuimos entre hip\'otesis mutuamente excluyentes (i.e.,~las hip\'otesis no pueden ocurrir simult\'aneamente).
...
No hay forma que, por la mera inspecci\'on exterior, otra persona sepa en cu\'al caja fue escondido el regalo. 
La \'unica manera ser\'ia abrir la caja para inspeccionar su interior.
%
La hip\'otesis de que el regalo se encuentra en la caja 1, la llamamos $r_1$. En tanto si se encontrara en la caja 2, ser\'ia $r_2$, o en la caja 3, $r_3$.
%
Luego, nuestro espacio de hip\'otesis es $R = \{r_1, r_2, r_3\}$ (Notaci\'on: $r_i \equiv r = i$, ``el regalo $r$ se encuentra en la posici\'on $i$'').
%
Por el principio de integridad, la creencia asignada a dos hip\'otesis $P(r_1 \, \cup \, r_2)$ debe ser la suma de creencias asignadas a cada una de ellas individualmente $P(r_1) + P(r_2)$.

Si, adem\'as, el espacio de hip\'otesis es exhaustivo, la suma de todas las creencias debe ser la unidad, pues estamos sumando la creencia que asignamos a todas las posibles hip\'otesis.
%
\begin{figure}[ht!]
\centering
...
%
Por ejemplo, si tuvi\'eramos el \emph{presentimiento} de que el regalo est\'a en la caja del medio, podr\'iamos expresar una certeza total asignando toda la creencia a la caja del medio. 
Esta situaci\'on se representa en la figura~\ref{fig:preferencia_total} en la que las hip\'otesis $r_1$ y $r_2$ no tienen asignada ninguna creencia y la creencia $r_2$ recoge la certeza total.
Por otro lado, tambi\'en podemos expresar una preferencia no total, pero s\'i m\'as fuerte, asignando mayor proporci\'on a la caja del medio que a las de los costados, como se ejemplifica en la figura~\ref{fig:preferencia_parcial}.
%
\begin{figure}[ht!]
 \centering
...
S\'i.
%
En pocas palabras debemos evitar mentir: no afirmar m\'as de lo que se sabe, sin dejar de decir todo lo que s\'i se sabe.
%
...
%
Seg\'un este principio, cuando no existe razones para tener preferencia por ninguna de las opciones debemos asignar la misma creencia a todos los eventos para no mentir (MaxEnt).
\todo[inline]{Saca la primera parte, no tiene nada que ver por como venis presentando los conceptos y no hace falta, quedate con presentar el principio de indiferencia y llama así a la subsection}

%
Esta distribuci\'on de creencias maximiza la incertidumbre (dividiendo la creencia en partes iguales).
\todo[inline]{ac\'a tenemos un problemilla: traes incertidumbre asociada a la noci\'on de maximimizaci\'on... para eso tenes que tenerla definida como una funci\'on y hasta ahora no la presentaste, creo que es ac\'a donde deber\'ias formalizar la noci\'on de incertidumbre para poder hablar de maximizarla... adem\'as que tendr\'ia que ser claro porque uno busca tener m\'as incertidumbre y no menos, resulta anti-intuitivo en los t\'erminos que venis usando}
...
\end{figure}
\todo[inline]{En la figura usas subfigure cuando no hace falta}

% Parrafo

El principio de indiferencia puede aplicarse tambi\'en cuando trabajamos con modelos causales multidimensionales.
%
Para ello, extenderemos el ejemplo del regalo.

\todo[inline]{deberia estar claro al terminar esta secci\'on porque separar la creencia en tres partes iguales es \emph{mejor}  que no hacerlo, para eso tenes que presentar los conceptos necesarios} 

\subsubsection{M\'axima incertidumbre dado modelos causales alternativos}

%
Supongamos que ahora nos piden que elijamos una caja $c \in \{1, 2, 3\}$ y despu\'es alguien nos muestra el interior de una caja $s \in \{1, 2, 3\}$ en la que no est\'a el regalo $r \in \{1, 2, 3\}$.
%
...
%
Este modelo causal se especifica de forma gr\'afica en la figura~\ref{fig:modelo_causal_1}.
%
Adem\'as, podemos sospechar que existe una segunda relaciones causal, seg\'un la cual la pista tampoco est\'a habilitada a abrir la caja que elegimos $s \neq c$ (figura \ref{fig:modelo_causal_2}).
...
%
Por el principio de integridad (o regla de la suma), la creencia de una variable individual (creencia marginal) no es m\'as que la suma de las creencias asignadas a todos los universos en los que esa variable est\'a presente.
%
...
%
Este \'ultimo sistema axiom\'atico es el m\'as utilizado por su simplicidad matem\'atica y en su versi\'on original dice,
%
\begin{quotation}
...
%
En las secciones~\ref{sec:volverAlFuturo} discutir\'e sobre el trabajo futuro en el \'area de la ciencia de datos causal bayesiana.
...
%
Aunque estos obst\'aculos han sido superado gracias a los avances computacionales y algorítmicos modernos, la inercia histórica hace que su desarrollo siga siendo incipiente, especialmente en países perif\'ericos.
%
...
La relevancia de la aplicación estricta de las reglas de la probabilidad contrasta con la realidad actual en la que el enfoque bayesiano es marginal incluso en las universidades m\'as importantes de la región, y pr\'acticamente inexistente en ciencias empíricas que no cuentan con formación específica en matem\'atica y programación.
%
...
%
En la siguiente figura~\ref{fig:papers_bayesianos} se detalla la cantidad de artículos científicos vinculados al enfoque bayesiano publicados cada a\~no con al menos una autoría con filiación en Am\'erica latina y el Caribe.
...
%
A\'un así, el auge de las redes neuronales profundas no produjo la aparición de un sistema de razonamiento bajo incertidumbre nuevo, con mejor desempe\~no que el de las reglas de la probabilidad.
%
...
La comunidad Bayes Plurinacional realizó con \'exito el Congreso Bayesiano Plurinacional 2023 los días 4, 5 y 6 de agosto en las instalaciones de la Secretaría de Ciencia y Tecnología de la provincia de Santiago del Estero, Argentina.
%
...
\item Gabriel A Alzamendi: \emph{M\'etodos basados en inferencia Bayesiana para la evaluación de los mecanísmos vocales de la fonación humana}.
...
\item Yasmín E Navarrete Díaz: \emph{Sistemas fr\'agiles: una formulación bayesiana de la teoría cu\'antica}.


\item Diego JR Sevilla: \emph{Regresión bayesiana para el an\'alisis de series temporales con apilamiento en Astronomía de rayos X}.
...
\item Albert R Ortiz: \emph{Aplicación de m\'etodos bayesianos en ingeniería estructural}.

\item Gonzalo Ríos: \emph{Bayesian Machine Learning Applied to Marketing}.

\item Sergio M Davis Irarr\'azabal: \emph{Probabilidad bayesiana a partir de la estimación plausible de cantidades}.

\item Tom\'as A Olego: \emph{M\'etodos robustos Bayesianos Generalizados para los modelos de regresión logística}.
...
Durante el congreso quedó de manifiesto una imperiosa necesidad de contar con mayor cantidad de instancias de formación teórica y pr\'actica en m\'etodos bayesianos.
%
...
%
Recientemente, la comunidad Bayes Plurinacional a enviado un proyecto a la organización de Inteligencia Artificial de Am\'erica Latina \emph{Khipu} para la realización de la Escuela Bayesiana Plurinacional 2024 en la Universidad Nacional de Salta, Argentina.

% Parrafo

\subsection{Taller ``Verdades emp\'iricas''}

Durante el congreso present\'e un taller sobre inferencia bayesiana denominado ``Verdades empíricas'', que hace referencia a la necesidad que tienen las ciencias empíricas de alcanzar verdades (o acuerdos intersubjetivos) en contextos de incertidumbre.
%
El taller estuvo dividido en 5 unidades y un hackatón.
%
Se puede acceder en línea a trav\'es de \url{https://github.com/BayesPlurinacional/tallerBP-2023}.
...
%
Allí se justifican las reglas de la probabilidad y se presentan los m\'etodos gr\'afico de especificación matem\'atica de los modelos probabilísticos y se aplican las reglas de la probabilidad para evaluar dos modelos causales alternativos.
%
...
%
Se eval\'uan diversos escenarios de estimación de efectos causales.
%
Finalmente definimos el concepto de teorías causales y mostramos cómo identificarlas a partir de intervenciones.
%
Aplicaciones: cómputo del efecto causal en modelos lineales bayesianos y evaluación de teorías causales alternativas con datos sint\'eticos.
...
%
Se revisa la estructura invariante del dato empírico y su equivalencia con el esquema emisor-receptor de la teoría de la información: unidad de an\'alisis (fuente), dimensiones (transmisor), procedimientos (perceptor), hipótesis indicadoras (receptor), y estimación (destino).
%
...
%
Se eval\'uan sistemas de comunicación con la realidad que difieren, no en los modelos causales, sino en los procedimientos que se usan para transmitir las se\~nales.
%
...
%
Se motiva el problema mediante los modelos habitualmente utilizados en la industria del videojuego, como TrueSkill, que usan el posterior de un evento como prior del siguiente, propagando la información en una \'unica dirección, del pasado al futuro.
%
...
%
Derivamos la propiedad epist\'emica que explica el funcionamiento de la teoría de la probabilidad.
%
...
%
Derivamos la propiedad de especiación, que le permite a grupos cooperativos aumentar un m\'as la tasa de crecimiento mediante la especialización cooperativa.
%
...
% %
% Al inscribirse, cada persona recibir\'a un array de longitud 2190 que representa la posición de los regalos en pasos temporales consecutivos.
% %
...
% %
% Tercero, deben actualizar sus creencia sobre la posición del regalo seg\'un la pista que reciben.
% %
% Este problema lleva a la pr\'actica los conceptos de probabilidad, de inferencia causal, de teoría de la información, de series temporales y de toma de decisiones.
...
%
Al mismo tiempo, quienes trabajan en ciencias sociales (y otras ciencias empíricas) proponen un enorme cantidad de argumentos causales en lenguaje natural, que si bien est\'an fundados en el conocimiento experto del \'area, al no estar especificados en t\'erminos probabilísticos no pueden ser evaluados correctamente.

% Parrafo

Existe así un complementariedad que todavía no est\'a siendo explotada: las ciencia de datos tiene la oportunidad de hacer uso de los modelos causales que se desarrollan en diversas ciencias empíricas, y las ciencias empíricas tienen la oportunidad de crear y evaluar modelos a la medida de cada problema específico~\cite{murphy-pmlBook1,murphy-pmlBook2, Bishop2013, Bishop2006}.
%
...
%
El desafío consiste en traducir matem\'aticamente argumentos expresados en lenguaje natural, integrar los diferentes elementos de prueba en un \'unico modelo probabilístico causal, computar correctamente el valor de verdad de los modelos causales alternativos.
%
