...
\todo[inline]{Aprovechar para aclarar que hipotesis refiere a conjunto de hipotesis elementales}
%
...
\todo[inline]{Ac\'a hay que explicar la notaci\'on grafica, es el primer lugar donde se la menciona, hay que explicar en concreto usando el ejemplo}
...
\todo[inline]{Arregl\'e el parrafo anterior para hacerlo coherente con haber introducido la pista un cachito antes, como estaba parec\'ia que recien ahora te aparece esa informaci\'on y en realidad, recien ahora la consideras}
%
...
\end{split}
\end{equation}
\todo[inline]{Aclarar en el modelo B que j no puede ser 1}
%
...
\todo[inline]{Por qu\'e solo del modelo B?}
%
...
\begin{figure}[ht!]
  ...
  \caption{Modelo $B$}
  \end{subfigure}
  \caption{Extensi\'on de los modelos $A$ y $B$ mediante la repeticiones de las restricciones entre hip\'otesis.
  \todo[inline]{El caption esta flojo, explicar un poco mas que se ve}}
...
\todo[inline]{Que significa ``desplegar''? Quizas una manera de decir esto es que las placas condensan o comprimen lo que queres mostrar}
%
...
\todo[inline]{?`Informacion? estas hablando de modelos sinteticos que repetis muchas veces y saltas a informacion que se incorpora? Suena descolgado... no querras decir que la instancia de cada repetici\'on es independiente del resto y listo?}
% Parrafp
...
\end{equation}
\todo[inline]{La productoria no ir\'ia hasta T que son la cantidad de muestreos?}
%
...
\todo[inline]{Ves, ac\'a traes que MB es mas restrictivo que MA y por eso usa los caminos posibles de MB para generar los datos, pero no lo dijiste antes ni lo justificas adecuadamente}
%
...
\todo[inline]{Aca dejas abierta una puerta medio extra\~na, porque tu m\'etodo de evaluaci\'on se basa en datos ``recortados'', es decir, que hay combinaciones que nunca vas a generar que el modelo A podr\'ia aceptar y que vos no vas a tener nunca entre tus datos. Creo que yo entiendo que la idea en este apartado es suponer como verdadero MB y generar los datos siguiendo MB como oraculo y despues evaluar cual de los dos funciona mejor. Bueno, eso tendrias que aclararlo bien cuando hablas de generar los datos sint\'eticos porque no se entiende.}
\todo[inline]{Ademas, podr\'ias ser una buena idea mostrar los numeros de como vas generando la probabilidad de cada modelo, pondr\'ia tambien una tabla con las hip\'otesis elementales y c\'omo va quedan el producto}
...
\end{equation*}
\todo[inline]{Ver notacion: en el texto es W e Y mayusculas, pero en la ecuaci\'on se vuelven minusculas}
...
\todo[inline]{Yo dejar\'ia bien claro que es cada cosa en la ecuaci\'on antes de hablar que significa la notaci\'on}
%
...
\todo[inline]{no se entiende la notaci\'on, quiere decir que son todos iguales entre s\'i e iguales al par ordenado de los infinitos? Aclara como leer eso}
%
...
\todo[inline]{Esto tambien esta raro, primero decis que son variables, pero luego decis que son constantes que conoces, y que estan en un rango entre los dos infinitos? no estar\'ia viendo que informaci\'on indica que este comprendido entre menos y mas infinito, esto no ser\'ia cualquier cosa?}
%
Las hip\'otesis $Y_0$ hasta $Y_{N-1}$ son observables.
\todo[inline]{De vuelta volves a las mayusculas, pero en las ecuaciones son minusculas. Ademas, traes el principal motivo del modelo lineal que es la relacion oculta entre observables y no observables y ni siquiera lo mencionas de antes... medio raro}
%
...
\todo[inline]{Esta oraci\'on es la clave de la motivaci\'on y aparece perdida en el medio...}
% Parrafo
...
\todo[inline]{?`Como que se ``generan''? Hay un generador de observables o queres decir que se supone que tienen un ruido con desvio $\beta$?}
%
La distribuci\'on respecto a la informaci\'on de la media y el desv\'io es la distribuci\'on gaussiana o Normal.
\todo[inline]{La informacion de la media y el desvio? Que significa?}
%
...
\end{equation}
\todo[inline]{explicar al menos de donde sale sigma}
%
...
\subsection{Posterior aproximado}
