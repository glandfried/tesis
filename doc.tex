...
\subsection{Incertidumbre y espacio de hipótesis}
...
\end{figure}
%
Si modificamos levemente el valor inicial de la población bajo un comportamiento caótico (por ejemplo de $0.5$ a $0.499$) obtenemos trayectorias que r\'apidamente divergen entre sí, siendo completamente diferentes una decena de pasos temporales (figura~\ref{fig:sensibilidad}).
%
\begin{figure}[ht!]
    \centering
...
\end{figure}
%
Es decir, a pesar de que el sistema sea determinista y conozcamos el mecanismo causal que lo produce, no ser capaces de conocer las condiciones iniciales con precisión \emph{infinitesimal} (por cuestiones de representación computacional o por ruido en la medición de datos) es suficiente para impedirnos predecir el comportamiento del sistema más allá de un breve lapso de tiempo.
%
A esta incertidumbre, que proviene de nuestra ignorancia, se la conoce como \emph{epistémica}.

%

Por otra parte, un fenómeno aleatorio puede definirse como el antónimo de un fenómeno determinista.
%
Por ejemplo, el concepto de libre albedr\'io adoptado en ciencias sociales afirma, por definición, que una persona expuesta ante un estado del universo determinado, bajo exactamente las mismas condiciones iniciales, tiene todav\'ia la capacidad de elegir una acci\'on.
%
La incertidumbre asociada a eventos aleatorios, que ocurrirían sin razón suficiente, se la conoce como \emph{ontológica}

% Parrafo

Sea porque el mundo natural admite la posibilidad de que un efecto surja sin una razón suficiente, o sea por nuestra incapacidad de conocer los modelos causales correctos o las condiciones iniciales con precisi\'on infinita, la incertidumbre nos impide describir el mundo mediante funciones deterministas $f(x)=y$, pues no tenemos una único valores $y$ para mapear ($f$) cada condición inicial ($x$), sino un conjunto de valores posibles $Y$.
%
A este conjunto se lo conoce como \emph{espacio de hipótesis}.

%

Por ejemplo, cuando sabemos que una persona escondió una regalo detrás de una caja entre tres, tenemos incertidumbre sobre la posición del regalo.
%
El regalo puede estar en la caja 1, $r_1$, en la caja 2, $r_2$, o en la caja 3 ,$r_3$.
%
Luego, nuestro espacio de hipótesis es $R = \{r_1, r_2, r_3\}$.
%
Tener en cuenta la siguiente notación, $r_i \equiv r = i$, ``el regalo $r$ se encuentra en al posición $i$''.


\subsection{Regla de la suma y las distribuciones de creencias}\label{sec:principio_integridad}

Aunque la incertidumbre nos impide describir el mundo mediante funciones deterministas, podemos describir nuestra creencias utilizando funciones de probabilidad.
%
Una función de probabilidad asigna una proporción de la creencia (un número entre 0 y 1) a cada posible hipótesis $h$.
%
La regla de la suma nos obliga a no perder ni crear creencias de forma espontánea cuando la distribuimos entre hip\'otesis mutuamente excluyentes (i.e.~si ocurre una hipótesis no puede ocurrir el resto).
%
Por ejemplo, la creencia asignada a dos hipótesis $P(h_1 \, \cup \, h_2)$ no es más que la suma de creencias asignadas a cada una de ellas $P(h_1 \, \cup \, h_2)= P(h_1) + P(h_2)$.
%
Si además el espacio de hipótesis es exhaustivo (contiene todas las posibles hipótesis), entonces debemos dividir toda nuestra creencia entre las opciones disponibles.
%
En el ejemplo del regalo, sabemos que hay solo 3 cajas donde puede estar escondido.
%
\begin{figure}[ht!]
\centering
...
\end{subfigure}
 \end{figure}
%
Bajo este principio hay infinitas formas de dividir la creencias que cumplen con la restricción de sumar 1.
%
Por ejemplo, si tenemos el presentimiento de que el regalo est\'a en la caja del medio podemos expresar una certeza total asignado toda la creencia a la caja del medio (figura~\ref{fig:preferencia_total}) o podemos expresar una preferencia parcial que asignando mayor proporci\'on a la caja del medio que a la de los costados (figura~\ref{fig:preferencia_parcial}).
%
\begin{figure}[ht!]
 \centering
...
\end{subfigure}
\caption{Dos distribuciones de creencias que tienen preferencia por la hip\'otesis de que el regalo se encuentra en la caja del medio.
 }
\label{fig:distribucion_de_creencias}
\end{figure}
% Parrafo
A las diferentes formas de dividir la creencia le llamamos \emph{distribuci\'on de creencias}.

\subsection{Principio de no mentir (MaxEnt)}\label{sec:principio_indiferencia}

La ciencia es una institución que tiene pretensión de alcanzar acuerdos intersubjetivos con validez universal (``verdades'').
%
% Las ciencias formales
% %
% Las ciencias empíricas, desde la física hasta las ciencias sociales, deben validar sus proposiciones en sistemas abiertos que contienen siempre algún grado de incertidumbre.
%
¿En contextos de incertidumbre, alguna de las infinitas distribuciones de creencias permite alcanzar estos acuerdos intersubjetivos?
%
Sí.
%
En pocas palabras debemos evitar mentir: no afirmar más de lo que se sabe, sin dejar de decir todo lo que sí se sabe.
%
Este principio se formalizó a mediados del siglo 20 como ``m\'axima entrop\'ia dadas restricciones'' o MaxEnt~\cite{Jaynes2003}.

% Parrafo

Antes de su definición formal, esta idea ya era discutida en el siglo 18 por los fundadores de la teoría de la probabilidad bajo el nombre de \emph{principio de indiferencia}~\cite{jaynes1984}.
%
Según este principio, cuando no existe razones para tener preferencia por ninguna de las opciones debemos asignar la misma probabilidad a todos los eventos.
%
Siguiendo el ejemplo, solo sabemos que el regalo se encuentra detrás de alguna de las tres cajas, y al no tener más información nos vemos en la necesidad de dividir la creencia en partes iguales para expresar una distribución de creencias que cumple con el principio de no mentir (MaxEnt).
%
Esta distribuci\'on de creencias maximiza la incertidumbre (dividiendo la creencia en partes iguales).
...
%          \node[const, below=of p2, yshift=-0.8cm] (title) {(c) Creencia honesta};
        ...
        \caption{Máxima incertidumbre}
        \label{fig:principio_de_indiferencia_c}
\end{subfigure}
\caption{Sin información a favor de ninguna caja, dividir la creencia en partes iguales representa la creencia honesta que cumple con el principio de no mentir  (MaxEnt).}
\label{fig:principio_de_indiferencia}
\end{figure}

% Parrafo

El principio de indiferencia puede aplicarse también cuando trabajamos con modelos causales multidimensionales.
%
Para ello, extenderemos el ejemplo del regalo.


\subsubsection{Maxima incertidumbre dado modelos causales alternativos}

%

Supongamos que ahora nos piden que elijamos una caja $c \in \{1, 2, 3\}$ y despu\'es alguien nos muestra el interior de una caja $s \in \{1, 2, 3\}$ en la que no está el regalo $r \in \{1, 2, 3\}$.
%
En este ejemplo tenemos tres variables ($r$, $c$ y $s$) y podemos imaginar al menos dos modelos causales alternativos entre las variables.
%
En un modelo causal (figura~\ref{fig:modelo_causal_1}) una única relación causal entre la posición del regalo $r$ y la pista $s$, pues sabemos que la pista no puede abrir la misma posición en la que encuentra el regalo $s \neq r$.
%
En un modelo causal alternativo (figura \ref{fig:modelo_causal_2}) se propone una segunda relaciones causal, que afirma que la pista tampoco puede abrir la caja que elegimos $s \neq c$.
...
Ahora, para determinar la distribución de creencias honestas debemos maximizar la incertidumbre incorporando la información que proveen los modelos causales en caso de que sea ciertos.
%
Cada uno de los modelos causales alternativos restringe un conjunto de universos paralelos.
%
