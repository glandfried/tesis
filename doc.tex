...
\end{equation*}
%
Seg\'un esta definici\'on puramente formal podemos definir arbitrariamente $\sum_{H_i \in \mathcal{H}} \text{cardinal}(H_i)^N$ conjuntos de datos distintos.
%
Sin embargo, en ciencias emp\'iricas el concepto de \emph{conjunto de datos} las hip\'otesis de investigaci\'on se eval\'uan sobre un \'unico conjunto de datos, que se determina en funci\'on de una dimensi\'on no capturada por la definici\'on formal.
...
%
La base emp\'irica est\'a compuesto por lo que llamamos evidencia formal que no se pone en duda (el sistemas de restricciones o modelos) y la evidencia emp\'irica (lo que coloquialmente llamamos datos).
%
Hemos visto en m\'as de un ejemplo que, en caso de que no exista un consenso sobre la evidencia formal (modelos o sistema de restricciones), ella pasan a formar parte de las hip\'otesis de investigaci\'on $\mathcal{H}$ y son evaluadas mediante la aplicaci\'on estricta de las reglas de la probabilidad.

% Parrafo

El conjunto de base emp\'irica depende
