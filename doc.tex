...
%
Luego, nuestro problema de investigaci\'on (o experimento) $\mathcal{H}$ consiste de tres hip\'otesis, $\mathcal{H} = \{H_R, H_C, H_S\}$, cada una con tres hipótesis elementales, $H_R=\{r_1, r_2, r_3\}$, $H_C=\{c_1, c_2, c_3\}$ y $H_S=\{s_1, s_2, s_3\}$.
%
...
%
En el modelo $A$ hay una única dependencia entre la hip\'otesis $H_R$ y la hip\'otesis $H_S$, $\forall\, i \in \{1,2,3\} \ P(s_i|r_i,M_A)=0$.
%
Debido a que en el modelo $M_A$ no existe ninguna dependencia que involucre a $H_C$, por simplicidad decidimos no incluir esa hipótesis en la red bayesiana.
...
%
El sistema de restricciones que incluye esta restricción y todas las restricciones del modelo $A$, le llamamos Modelo $B$ ($M_B$).
%
...
\todo[inline, color=gray]{No entendí}
...
\begin{figure}[ht!]
  ...
  \caption{Modelo $B$}
  \end{subfigure}
  \caption{Extensi\'on de los modelos $A$ y $B$ mediante la repeticiones de las restricciones entre hip\'otesis. La notación de redes bayesianas especifica las repeticiones mediante placas. Las placas son como pilas de naipes, que difieren entre sí solo por el número indicado en el subíndice, $t \in \{1,\dots,T\}$.
  }
  \label{fig_modelos_con_repeticiones}
\end{figure}
%
Las placas son como naipes repetidos, uno arriba del otro, que difieren solamente en el número indicado por el subíndice.
%
...
%
El hecho de que no est\'en conectadas hace que sean independientes entre s\'i, las hipótesis observadas en una placa no afectan las probabilidades en el resto de las placas.
...
%
Luego calculamos las predicciones de los modelos (o evidencia) usando el lenguaje de programación \texttt{Julia}.
%
...
\end{equation}
%
donde $\sigma$ representa la incertidumbre a priori de las hipótesis $W_0$ a $W_D$.
%
...
El balance natural de las reglas de la probabilidad est\'a relacionado con el hecho de que las distribuciones de probabilidad debe sumar o integrar 1.
%
...
\section{Cuestiones relativas a la operacionalizaci\'on de proposiciones emp\'iricas} \label{sec:base_empirica_metodo}
...
%
%Quisi\'eramos que el mensaje que recibimos de la realidad sea lo m\'as preciso posible.
...
En la secci\'on \emph{\nameref{sec:reglas_de_la_probabilidad}} definimos el conjunto de datos sobre el que se eval\'uan las hip\'otesis de investigaci\'on $H_i \in \mathcal{H}$ mediante la f\'ormula general del teorema de Bayes \eqref{eq:teorema_bayes} como un subconjunto arbitrario de hip\'otesis elementales, $D \subset \textbf{H}$, tal que $P(Datos)>0$.
%
...
%
En la secci\'on~\ref{} operacionalizaremos la funci\'on proposicional \emph{habilidad}.


\subsection{La estructura invariante de las proposiciones emp\'iricas} \label{sec:estructura_invariante}
...
%
La definici\'on precisa de la funci\'on proposicional, sin embargo, radica en la operaci\'on emp\'irica que ejecutamos para vincular el concepto $f(x)$ con el valor $y$.
%
Sin embargo, pocas veces hacemos expl\'icita la operaci\'on subyacente, por lo que dos personas pueden usar la misma palabra para expresar conceptos que en el fondo son completamente distintos.

% Parrafo

En el lenguaje de la Teor\'ia de la Probabilidad, las funciones proposicionales $f(x)$ representan las hip\'otesis de investigaci\'on y las proposiciones emp\'iricas son sus distribuciones de probabilidad a posteriori, luego de incorporar hip\'otesis elementales como datos a la base emp\'irica.
%
El epistem\'ologo Juan Samaja~\cite{Samaja1999} propone una estructura que ser\'ia invariante a toda proposici\'on emp\'irica sobre hip\'otesis de investigaci\'on oculta.
%
Esta estructura tendr\'ia, al menos, los siguientes elementos: la una hip\'otesis de investigaci\'on oculta de inter\'es ($H$), una hip\'otesis de investigaci\'on observable ($D$) que transmite indirectamente informaci\'on  de la hip\'otesis oculta de inter\'es, un procedimientos de medici\'on ($P$) que detecta se\~nales de la hip\'otesis observable, la que se incorpora a la base emp\'irica ($B$) mediante la cual se infiere o estima el valor de verdad ($V$) o probabilidad de las hip\'otesis elementales de la hip\'otesis oculta $H$.
%
Estos elementos conectan la parte abstracta de las proposiciones emp\'iricas (la funci\'on proposicional) con su parte emp\'irica, el procedimiento ejecutado en la realidad.
%
\begin{figure}[H]
    \centering
\tikz{
...
\caption{Los niveles de abstracci\'on de la estructura invariante de las proposiciones emp\'iricas propuesta por Samaja. Los nombres originales utilizados por Samaja fueron renombrados bajo el lenguaje de la Teor\'ia de la Probabilidad.}
\end{figure}

% Parrafo

Las proposiciones emp\'iricas est\'an cargadas de supuestos a priori.
%
Especificar la hip\'otesis oculta, por ejemplo $H:=$ \emph{habilidad}(Messi), involucra suponer a priori que alguna de sus hip\'otesis elementales coincide con la realidad oculta, pues por definici\'on las hip\'otesis de investigaci\'on son conjuntos mutuamente excluyentes y exhaustivos.
%
Por otra parte, la elecci\'on de la hip\'otesis observable, por ejemplo $D:=$ Ganar/Perder, se realiza en base a alg\'un modelo del mundo que vincula de alguna manera las habilidades ocultas con los resultados observables Ganar/Perder.
%
El dise\~no del procedimiento de observaci\'on, por ejemplo $P:=$ \emph{Scraper} de \texttt{fifa.com}, se basa en la supuesta veracidad de la informaci\'on provista por la Federaci\'on Internacional de F\'utbol Asociado.
%
La elecci\'on de un modelo de estimaci\'on de habilidad espec\'ifico como base emp\'irica $B:=$ se basa en el nivel de aceptaci\'on que tiene al interior de la comunidad acad\'emica, de la industria del video juego y del deporte.
%
...
\subsection{Tasa de informaci\'on de los sistemas de informaci\'on} \label{sec:tasa_de_informacion}

Una misma funci\'on proposicional, como por ejemplo \emph{habilidad}(Messi), puede tener varias operacionalizaciones alternativas si consideramos distintas hip\'otesis observables $D$, si consideramos distintos procedimiento $P$, o si consideramos distintas base emp\'iricas $B$.
%
La aplicaci\'on estricta de las reglas de la probabilidad nos permite comparar el desempe\~no de proposiciones emp\'iricas alternativas.
%
De hecho, en la secci\'on~\ref{sec:evaluacion_honesta_de_modelos} hemos visto un ejemplo simple que inclu\'ia inicialmente tres hip\'otesis de investigaci\'on, $\mathcal{H}=\{H_R, H_S, H_C\}$, la posici\'on del regalo $H_R$, la caja que se abre $H_S$, y la caja elegida $H_C$.
%
En ese ejemplo consideramos dos operacionalizaciones distintas de la proposici\'on emp\'irica $P(H_R,H_S|c_1,M_k)$, cuando consideramos dos modelos alternativos $M = \{M_A, M_B\}$.
%
Para realizar la comparaci\'on ofrecimos una extensi\'on de los modelos (figura~\ref{fig_modelos_con_repeticiones}) donde se especifica el mismo sistema de restricciones entre hip\'otesis para una cantidad indefinida de repeticiones $T$.
%
...
\end{equation*}
%
Luego, preferimos las operacionalizaciones de las proposiciones emp\'iricas que minimizar la tasa de informaci\'on que recibimos de la realidad, maximizando la predicci\'on conjunta que hacemos de los datos.
%
...
\section{Operacionalizaci\'on de \emph{habilidad}} \label{sec:operacionalizacion_habilidad}
...
%
Dado que la habilidad es una hip\'otesis ocultas, debemos estimarla a partir de sus consecuencias observables directas: el \'exito o fracaso en en la resoluci\'on de problemas y competencias.
%
...
%
Los primeros modelos causales, propuestos hace casi un siglo por~\cite{Thurstone1927} y~\cite{Zermelo1929}, supon\'ian que la probabilidad de un resultado observado $r$ depende del rendimiento $p$ (\emph{performance}) del agente $i$ y de su oponente $j$, $P(\, r \,|\, p_i, \, p_j \,)$.
%
El campo sigui\'o progresando con los trabajos de Bradley y Terry \cite{Bradley1952} y~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c} Mosteller, que permiti\'o a Arpad Elo~\cite{Elo2008} desarrollar una metodolog\'ia de estimaci\'on de habilidad para la Federaci\'on de Ajedrez de los Estados Unidos (USCF), adoptada hasta el d\'ia de hoy por la Federaci\'on Internacional de Ajedrez (FIDE).

% Parrafo

Hasta las v\'isperas del siglo 21, la estimaci\'on de habilidad se resolv\'ian utilizando funciones de costo ad hoc.
%
...
%
Actualmente, los modelos bayesianos de habilidad Glicko \cite{glickman2001} y TrueSkill \cite{Herbrich2007} son los m\'as utilizados por la industria del video juego, y al inicio de esta tesis eran considerados est\'andares para la estimaci\'on de habilidad en el tiempo.
%
Gracias a la aceptaci\'on generalizada, las estimaciones de habilidad de estos modelos ser\'an consideradas base emp\'irica metodol\'ogica a lo largo de esta tesis sobre la cual evaluaremos los posibles factores de aprendizaje (figura~\ref{fig:base_empirica_tesis}).
%
\begin{figure}[ht!]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
    ...
    \end{subfigure}
    \caption{Niveles de base emp\'irica de esta tesis. La base emp\'irica epistemol\'ogica (en negro) son lo resultados (Ganar/Perder) observados directamente en las bases de datos de comunidades de video juegos en l\'inea. La base emp\'irica metodol\'ogica (en gris) son las estimaciones de habilidad realizadas con los modelos bayesianos estados del arte en la industria del video juego. En base a esta informaci\'on estudiaremos algunos posibles factores de aprendizaje (en blanco) no considerados previamente por la literatura de aprendizaje humano.}
    \label{fig:base_empirica_tesis}
\end{figure}
% Pararfo
...
%
El modelo causal de una partida de ajedrez supone que las habilidades (\emph{skills}) ocultas de dos agentes $i$ y $j$, $H_{S_i} = H_{S_j} = (-\infty,\infty)$, generan el resultado observable $H_R= \{\text{Ganar}, \text{Perder} \}$ a trav\'es de la diferencia $H_D = (-\infty,\infty)$ de los desempe\~nos aleatorios $H_{P_i} = H_{P_j} = (-\infty,\infty)$.
%
Luego, el problema de investigaci\'on contiene las siguientes hip\'otesis, $\mathcal{H} = \{H_{S_i}, H_{S_j}, H_{P_i}, H_{P_j}, H_D, H_R \}$.
%
En la figura~\ref{fig:generative_model_elo} especificamos la versi\'on ``gaussiana'' del modelo Elo (descrita en el art\'iculo~\cite{Herbrich2007}) utilizando la notaci\'on de redes bayesianas.
%
Debido a que durante el siglo 20 se seleccionaba una \'unica hip\'otesis elemental de habilidad $H_{S_i} = \{ s_i \}$, $H_{S_j} = \{ s_j \}$, (en base a una funci\'on de costo ad hoc que revisaremos m\'as adelante), especificamos las hip\'otesis de habailidad mediante puntos negros, que representan constantes.
%
\begin{figure}[ht!]
  \centering
...
%
Una vez que se generan los desempe\~nos, el resultado queda determinado pues seg\'un el modelo gana el agente con obtuvo mayor desempe\~no, $r = (p_i > p_j)$.
%
El par\'ametro $\beta^2$, es el mismo para todos los agentes, y act\'ua como la escala de las estimaciones: habilidades a un $\beta$ de distancia, $s_i-s_j=\beta$, implica 76\% de probabilidad de ganar, $P(p_i > p_j|s_i,s_j)\approx0.76$, independientemente del valor absoluto de las habilidad $s_i$ y $s_j$.
%
En t\'erminos gr\'aficos, podemos representar esa probabilidad como el volumen indicado en la figura \ref{fig:probaOfWin_2D}.

% Parrafo

La metodolog\'ia desarrollada por Arpad Elo implementa un procedimiento ad hoc de selecci\'on de las hip\'otesis de habilidad basado en las predicciones del modelo (continuamos con la versi\'on provista en el art\'iculo~\cite{Herbrich2007}).
%
...
%
Sea el resultado $r \in H_R$, la ``sorpresa'' se define como,
%
\begin{equation*}
...
\begin{equation}\label{eq:elo_update}
 s_{\text{winner}}^{\text{new}} = s_{\text{winner}}^{\text{old}} + \Delta \ \ \ \ \ s_{\text{loser}}^{\text{new}} = s_{\text{loser}}^{\text{old}} - \Delta
\end{equation}
%
...
Por un lado, la regla de actualizaci\'on (Ec.~\eqref{eq:elo_update}) es sim\'etrica, lo que gana un agente lo pierde el otro.
%
...
%
En general se usa una incertidumbre que es 3 veces el desv\'io estandar del desempe\~no, $\beta$.
...
Calculada la distribuci\'on de probabilidad marginal de los desempe\~nos, la predicci\'on a priori del resultado de la partida entre dos individuos $i$ y $j$ se corresponde nuevamente con el volumen de un lado y del otro de la isol\'inea $p_i = p_j$ visto en la figura~\ref{fig:probaOfWin_2D}, .
%
...
\subsection{Posterior aproximado y extensión del modelo}
...
\end{align}
%
Con dos equipos, la funci\'on exacta de actualizaci\'on posterior es (más detalles en el capítulo \ref{ch:ttt}),
%
\begin{equation}
...
No hay m\'as.
%
A diferencia de lo que ocurre con la mayoría de funciones de costo ad-hoc, la aplicaci\'on estricta de las reglas de la probabilidad actualiza la creencia de una hipótesis mediante un proceso multiplicativo, una secuencia de predicciones.
%
...
%
De forma similar a lo que le ocurre a las especies en evolución, la hipótesis que se extinguen nunca más vuelven.
...
%
En la siguiente ecuación ofrecemos un esquema del isomorfismo propuesto.
%
...
\end{equation*}
%
La noción de \emph{función de costo epistémico-evolutiva} hace referencia a este isomorfismo.
%
Luego, comprender las propiedades de esta función de costo parece esencial para comprender los mecanismos fundamentales del aprendizaje.
...
Para estudiar las propiedades de la función de costo epistémico-evolutiva, veamos qu\'e le ocurre a las variantes sujetas a procesos multiplicativos.
%
En base a los ejemplos propuestos por John Kelly~\cite{kelly1956} y Ole Peters~\cite{peters2019-ergodicityEconomics}, supongamos que una casa de apuestas ofrece pagos $Q_c > 0$ y $Q_s > 0$ cuando una moneda sale Cara y Seca respectivamente y que los individuos se ven obligados a apostar en cada paso temporal todos sus recursos, asignando una proporci\'on $b_c = b$ a Cara y $b_s = 1 - b$ a Seca, con $b \in [0,1]$.
%
Sea $\omega_{t}$ los recursos totales en el tiempo $t$.
%
Luego, los recursos que se apuestan en el tiempo $t$ a Cara $w_t \, b$, se pierden si sale Seca, o se multiplican por $Q_c$ si sale Cara.
%
De forma similar, los recursos que se apuestan en el tiempo $t$ a Seca $w_t \, (1-b)$, se pierden si sale Cara, o se multiplican por $Q_s$ si sale Seca.
%
En particular, sean nuestros recursos iniciales $\omega_0$, los recursos que conseguimos con la apuesta en dos pasos temporales $\omega_2(b)$, luego de obtener una Cara y una Seca, son
%
 \begin{equation*}
\begin{split}
...
\end{equation*}
%
donde los recursos obtenidos en el primer paso temporal, $\omega_1(b)$, se generaron luego de que los recursos apostados a Cara $\omega_0\,b$ fuera multiplicada por el pago correspondiente a Cara, $Q_c$.
%
De forma similar, los recursos en el segundo paso temporal, $\omega_1(b)$, se generaron luego de que los recursos apostados a Seca $\omega_1(b)\,(1-b)$ fuera multiplicada por el pago correspondiente a Seca, $Q_s$.
%
En términos generales, los recursos que conseguimos con una apuesta $b$ en $T$ pasos temporales es,
%
\begin{equation*}
...
%
De forma miliar a lo que ocurre con la función de costo epistémico-evolutiva, los recursos se actualizan siguiendo un proceso multiplicativo.
%
Como ocurre en la evaluación de hipótesis en probabilidad \eqref{eq:funcion_de_costo} o la selección de formas de vida en evolución, introducir un único cero en la secuencia produce la pérdida total e irreversible de los recursos.
%
Esto hace que no debemos apostar todo los recursos a una de las opciones, ni a Cara $b=1$ ni a Seca $b=0$.
%
Pero ¿Cuál apuesta maximiza la tasa de crecimiento de los recursos?


\subsection{Propiedad epist\'emica.} \label{sec:propiedad_epistemica}

Supongamos que la casa de apuestas ofrece $Q_c = 3$ por Caras y $Q_s = 1.2$ por Secas por una moneda no sesgada, con probabilidad $p=0.5$.
%
En este escenario existe la tendencia a apostar más a Caras que Secas.
%
...
%
Este descubrimiento fue realizado por Kelly~\cite{kelly1956} y en su manuscrito también aparece reportado con un signo de admiración.
%
Pero, ¿Cuál es la apuesta $b$ \'optima?

%

Para calcular el valor de la tasa de crecimiento de los recursos en el tiempo $r(b)$, debemos analizar la siguiente función,
%
\begin{equation}
\begin{split}
...
%
Debido a que lo pagos que ofrece la casa de apuestas no importan, el el valor óptimo de $b$ es,
%
\begin{equation}
\begin{split}
\overbrace{\underset{b}{\text{arg max}} \phantom{T} \log r(b)}^{\hfrac{\text{\scriptsize \En{Optimal}\Es{Apuesta}}}{\text{\scriptsize \En{bet}\Es{\'optima}}}} \ = \underset{b}{\text{arg max}} \phantom{T} \overbrace{\log p \log b + (1-p) \log (1-b)}^{-\text{Entropía cruzada}} \overset{*}{=} p
\end{split}
\end{equation}
%
Es decir, maximizar la tasa de crecimiento de la apuesta es equivalente a minimizar la entropía cruzada.
%
...
%
A esta propiedad la llamamos \emph{propiedad epist\'emica} debido a que es la que permite a la teor\'ia de la probabilidad adquirir conocimiento sobre el mundo: al evaluar las hip\'otesis elementales en base al producto de las sorpresas se produce una ventaja a favor de las hip\'otesis que diversifican la predicci\'on en la misma proporci\'on que la frecuencia observada.
%
...
%En este escenario, con pagos $Q_c=3$, $Q_s=1.2$ y una moneda Si sabemos que la moneda es no sesgada, lo mejor que podemos hacer a largo plazo es dividir los recursos en partes iguales, $b = p = 0.5$.
En este caso, si calculamos la tasa de crecimiento veremos que los recursos caen a una tasa cercana a $5\%$ ($r(b) \approx 0.95$), pues las ganancias de 50\% que obtenemos con Caras ($1.5 = Q_c \, b^*$) no alcanza para compensar las ca\'idas de 40\% que sufrimos con Secas ($0.6 = Q_s \, (1-b^*)$).
%
En la figura~\ref{fig:ergodicity_individual_trayectories}, las curvas de colores muestran las trayectoria de los recursos individuales en el tiempo.
...
%
Por ejemplo, si los individuos de un grupo de tama\~no $N$ redistribuyen los recursos en partes iguales luego de cada apuesta, la riqueza de cada uno pasa a ser el promedio de todos los recursos luego de que $m_c$ y $m_s$ individuos obtienen Cara y Seca respectivamente, con $m_c + m_s = N$.
%
\begin{equation}
\begin{split}
...
%
\'Esta es la \emph{propiedad evolutiva}.
%
...
%
\'Esta es la \emph{propiedad de especiaci\'on}.
%
