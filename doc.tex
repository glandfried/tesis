...
\chapter{Metodología}\label{ch:metodo}


Desde el descubrimiento de las reglas de la probabilidad a finales del siglo 18 hasta el presente, la teoría de la probabilidad es el enfoque más utilizado para razonar en contextos de incertidumbre.
%
...
%
El sistema axiomático de Kolmogorov \cite{Kolmogorov1950} en cambio, incluye una caso particular de la regla del producto como axioma e incorpora la regla del producto como una definición complementaria que cumple con los axiomas.
%
Por su elegancia matemática el sistemas axiom\'atico de Kolmogorov es el m\'as utilizado.
%
En la sección \ref{} presentamos la teoría elemental de la probabilidad siguiendo los axiomas de Kolmogorov, en su versión simple.
%
En la sección \ref{} revisaremos algunas consecuencias de la aplicación estricta de las reglas de la probabilidad.
%
En la sección \ref{} ofreceros una operacionalización el dato fundamental de esta tesis (la habilidad de los individuos en el tiempo o aprendizaje) basada en el modelo de estimación de habilidad de la industria del video juego.

\section{Teoría elemental de la probabilidad}

\subsection{Sistema axiomático}

En la sección 1 del capítulo 1 ``Elementary Theory Of Probability'' de su libro (traducción al ingles~\cite{Kolmogorov1950} de su manuscrito original) se presenta el siguiente sistema axiomático, definido sobre elementos discretos.
%
\begin{quotation}
...
%
El enfoque bayesiano de la probabilidad lo suele denominar \emph{espacio de hipótesis} debido a que este mismo sistema axiomático se lo utiliza para asignar y actualizar probabilidades sobre eventos que nos son necesariamente aleatorios.
%
Kologorov reconoce esta interpretación cuando más adelante dice ``$H_1, H_2, \dots, H_2$ are often called \emph{hipótesis}''.

% Parrafo

La forma más simple de crear un campo de probabilidad (\emph{field of probability}) es tomar un conjunto arbitrario de hipótesis elementales $\textbf{H} = \{h_1, h_2, \dots, h_k\}$
% %
...
% %
% Una vez escondido, la posición del regalo permanece fija (no es aleatoria).
% %
% Las hip\'otesis elementales ``el regalo se encuentra en la caja $i$'' las llamamos $r_i$.
% %
% Luego el conjunto de hipótesis elementales es $\textbf{H} = \{r_1, r_2, r_3\}$.
% %
y asignarle una probabilidad arbitraria a cada hipótesis elemental $\{p_1, p_2, \dots p_k\}$ tal que la suma $p_1 + p_2 + \dots + p_k = 1$.

% Parrafo

\subsection{Terminología basada en la teoría de conjuntos}

En la sección 3 ``Notes on Terminology'' Kolmogorv aclara algunos aspectos terminológicos.
%
\begin{quotation}
...
En el sistema axiomático de Kolmogorov un caso particular de la regla de la suma de la teoría de la probabilidad aparece incluida directamente como axioma 5.
%
La regla del producto (o probabilidad condicional) en cambio se incluye mediante una definición complementaria que cumple con los axiomas.
%
En la sección 4 ``Immediate Corollaries of the Axioms; Conditional Probabilities; Theorem of Bayes'' dice,
%
\begin{quotation}
...
Sea el experimento $\mathcal{H}= \{ H_1, H_2, \dots, H_n \}$ (esto asume que las hipótesis $H_i$ son mutuamente exclusivas) y sea $D \subset \textbf{H}$ un conjunto arbitrario.
%
...
%
Luego definimos la fórmula general del teorema de Bayes como,
%
...
\section{Aplicación estricta de las reglas de la probabilidad}

\subsection{Acuerdos intersubjetivos en contextos de incertidumbre}



\subsection{El costo computacional de las reglas de la probabilidad}

El problema hist\'orico para la aplicaci\'on estricta de las reglas de la probabilidad ha sido el costo computacional asociado a la evaluaci\'on completa del conjunto de hip\'otesis que se ponen a prueba cuando se buscan  $\mathcal{H}$.
%
Con hipótesis que toman discretos, por ejemplo, el tama\~no de la distribuci\'on conjunta crece de forma exponencial en funci\'on de la cantidad de variable $n$ y la cantidad de estados $e$ que pueden adquirir las variables $e^n-1$.
%
...
\section{Cuestiones relativas a la operacionalización del los datos} \label{sec:base_empirica_metodo}
...
\subsection{Tasa de informaci\'on de las operacionalizaciones de datos} \label{sec:tasa_de_informacion}
...
\section{Operacionalización del dato \emph{habilidad}.}
...
\chapter{Discusión 1. Incertidumbre e informaci\'on}\label{ch:Incertidumbre}
...
\todo[inline]{Saca la primera parte, no tiene nada que ver por como venis presentando los conceptos y no hace falta, quedate con presentar el principio de indiferencia y llama as\'i a la subsection}
...
%
Sigamos con el ejemplo del regalo escondido dentro de una de tres cajas.
Tenemos que eligir una caja $c \in \{1, 2, 3\}$, pero ahora un or\'aculo nos indica que en el interior de una de las cajas $s \in \{1, 2, 3\}$ \textbf{no est\'a} el regalo $r \in \{1, 2, 3\}$.
\todo[inline]{Por qu\'e aca cambias la notaci\'on? no hay motivos para saltar de nomeclatura. Ya se que a vos te parece trivial, pero no se justifica y puede confundir. En todo caso, empareja para que lo anterior sea consistente con esta si es que la notacion a partir de aca es mejor o mas clara para lo que sigue.}
%
En este ejemplo tenemos tres variables: la posici\'on del regalo, $r$; la caja que elegimos, $c$; y la pista que recibimos $s$.
%
En base a la descripci\'on del juego, sabemos que al menos hay una relaci\'on causal entre la posici\'on del regalo $r$ y la pista $s$, pues la pista \textbf{no puede coincidir} con la misma posici\'on en la que encuentra el regalo, es decir $s \neq r$.
\todo[inline]{Esta bien que es un ejemplo, pero que sucede con nuestra selecci\'on en el modelo A, no se lo considera? Se supone que no importa? Algo habr\'ia que decir al respecto para formular el modelo A, sino queda raro. No servir\'ia mejor comenzar por un modelo en el que elegimos la posici\'on y otro en el que se agrega la pista? Queda raro que el juego sea elegir la caja y en el modelo A eso no est\'a contemplado.}
%
Este modelo causal se especifica de forma gr\'afica en la figura~\ref{fig:modelo_causal_1}.
\todo[inline]{Tenes que presentar la notaci\'on gr\'afica: qu\'e significan los c\'irculos y qu\'e las flechas}
%
Adem\'as, podemos sospechar que existe una segunda relaci\'on causal, seg\'un la cual la pista tampoco podr\'ia coincidir con la caja que elegimos, es decir $s \neq c$.
La figura~\ref{fig:modelo_causal_2} ejemplifica este modelo alternativo que incluye las dos relaciones mencionadas.
\todo[inline]{En general, habr\'ia que explicar la causalidad en estos modelos, yo no entiendo como se deducen esas relaciones del texto que pones, no est\'a bien explicado}
...
\begin{figure}[ht!]
  ...
  \todo[inline]{Fijate que en el texto hablas de \emph{dependencia} pero antes te referis a causalidad... al menos en espa\~nol esas palabras tienen distinta fuerza, tendr\'ias que definir bien a que llamas la relaci\'on causa-efecto en este ejemplo, a m\'i no me queda claro}q
  }
  \label{fig:modelos_causales}
\end{figure}


Para determinar la distribuci\'on de creencias, debemos maximizar la incertidumbre incorporando la informaci\'on que proveen los modelos causales.
%
Cada uno de los modelos causales alternativos restringe un conjunto de universos posibles.
%
Aplicando el principio de indiferencia, dividimos la creencia en partes iguales en cada una de las bifurcaciones de los universos paralelos que producen los modelos causales.
%
En la figura~\ref{fig:caminos} mostramos c\'omo se divide la creencia a través de los universos paralelos de ambos modelos causales.
%
Los caminos que van de la ra\'iz a la hoja representan universos posibles del modelo causal.
%
El estado del universo se representa con c\'irculos.
\todo[inline]{Que es el \emph{estado}? Donde lo definiste antes? El estado son las hipotesis que vas dando por ciertas en ese camino causal? Hay que explicar las cosas}
%
La ra\'iz contiene toda la creencia, la cual se divide en cada una de las bifurcaciones hasta llegar a las hojas.
%
De esta forma se maximiza la incertidumbre dada la informaci\'on provista por los modelos causales.
%
...
\node[latent,below=of b0,yshift=0.7cm, xshift=-2cm] (r1) {$r=1$};
\node[latent,below=of b0,yshift=0.7cm] (r2) {$r=2$};
\node[latent,below=of b0,yshift=0.7cm, xshift=2cm] (r3) {$r=3$};
...
\node[latent,below=of br1,yshift=0.7cm, xshift=-0.5cm] (r1d2) {$s=2$};
\node[latent,below=of br1,yshift=0.7cm, xshift=0.5cm] (r1d3) {$s=3$};
...
\node[latent,below=of br2,yshift=0.7cm, xshift=-0.5cm] (r2d1) {$s=1$};
\node[latent,below=of br2,yshift=0.7cm, xshift=0.5cm] (r2d3) {$s=3$};
\node[latent,below=of br3,yshift=0.7cm, xshift=-0.5cm] (r3d1) {$s=1$};
\node[latent,below=of br3,yshift=0.7cm, xshift=0.5cm] (r3d2) {$s=2$};
...
\node[latent,below=of bc11,yshift=0.7cm, xshift=-0.5cm] (r1d2) {$s=2$};
\node[latent,below=of bc11,yshift=0.7cm, xshift=0.5cm] (r1d3) {$s=3$};
\node[latent,below=of bc12,yshift=0.7cm] (r2d3) {$s=3$};
\node[latent,below=of bc13,yshift=0.7cm] (r3d2) {$s=2$};
...
\todo[inline]{Fijate que ajust\'e la notaci\'on de la figura para que sea coherente con lo que dec\'is al principio. Adem\'as, yo te sugiero poner la distribuci\'on de probabilidad para separar en casos por nivel y dejar la parte de obtener la condicional en el texto. Por ejemplo en el modelo A para el camino r=1, yo separar\'ia 1/2 y 1/2 en cada opcion de s sin incluir la multiplicaci\'on por la probablidad de que pase r=1, eso lo discutir\'ia directamente en el texto. Ademas, tenes que aclarar que pasa con las bifurcaciones que no incluis, se entienden que son caminos de creencia nula porque no pueden ocurrir, pero no decis nada y despues aparecen en la tabla de abajo todas las combinaciones, lo mas prolijo sería incluirlas aca con 0.}}
\label{fig:caminos}
\end{figure}
%
...
\todo[inline]{Aca tenes que explicar que luego en el siguiente nivel volves a aplicar el principio de indiferencia y separar la creencia total de ese universo en los dos casos posible y le asignas la mitad a cada una. Luego tendr\'ias que presentar la noci\'on de condicional si queres hablar de la prob de cada caso. Ademas, en realidad en cada caso se abren tres opciones, no dos, solo que una es un camino prohibido o con creencia nula.}
%
...
%
La creencia que llega a las hojas representa la \emph{creencia conjunta} de que el regalo se encuentre en la caja $i$ y la pista se\~nale la caja $j$, $P(r=i, s=j|\text{Modelo})$.
%
\begin{table}[ht!]
\centering
$P(r=i, s=j | \text{Modelo A})$ \hspace{1.8cm} $P(r=i, s=j | \text{Modelo B})$ \\[0.1cm]
 ...
 & \, $r=1$ \, &  \, $r=2$ \, & \, $r=3$ \,  \\ \hline
  ...
  $s=3$ & $1/6$ & $1/3$ & $0$  \\ \hline
  \end{tabular}
  \caption{Creencia conjunta para cada uno de los modelos causales. Las celdas representan universos paralelos y el valor la creencia asignada a cada una de ellas.
  \todo[inline]{Tambien ajust\'e la notaci\'on en la tabla para que sea coherente con lo anterior. Ojo!}}
  \label{tab:creencia_conjunta}
\end{table}
%
Esta informaci\'on la podemos resumir en forma de tabla~\ref{tab:creencia_conjunta} que representa la creencia asignada a cada uno de los universos paralelos.
...
En la tabla~\ref{tab:creencia_conjunta} tenemos la creencia conjunta de que ocurra cada combinaci\'on posible de variables.
%
Por el principio de integridad (o regla de la suma), la creencia de una variable individual (creencia marginal) no es m\'as que la suma de las creencias asignadas en todos los universos en los que esa variable est\'a presente.
Por ejemplo, podemos extraer la creencia individual de que la pista $s$ est\'e asignada a una posici\'on determinada:
%
\begin{equation} \label{eq:creencia_marginal}
\begin{split}
P(s=j|\text{Modelo A}) = \sum_{i \in \{1,2,3\}} P(r=i, s=j|\text{Modelo A}) = 1/3 \\  P(s=j|\text{Modelo B}) = \sum_{i \in \{1,2,3\}} P(r=i, s=j|\text{Modelo B}) = 1/2
\end{split}
\end{equation}
\todo[inline]{Falta discutir en palabras que significan estas ecuaciones, por ejemplo, dar la explicaci\'on  de que significa la diferencia en cada modelo. Tenes que decir en donde se mueve $i$ en la sumatoria. Adem\'as, segun la tabla de antes en el modelo B si s=1, la marginal es 0... esta bien?}
% Parrafo

Lo mismo ocurre con cualquiera de las variables de un modelo multidimensional.
\todo[inline]{Y este comentario descolgado? A que apunta?}
%
En ambos modelos, la creencia marginal sobre el regalo es $1/3$.
\todo[inline]{No te cuesta nada decir que es sumar por cada columna de la tabla en cada modelo, no? Ademas, no te sorprende que aun teniendo una pista, la marginal sobre el regalo siga siendo 1/3? A mi me sorprende. Algo tenes que decir}
%
\begin{figure}[H]
\centering
\tikz{ %
...
\end{figure}
%
La cerradura representa la variable $c$ (la elecci\'on de la caja) que por simplicidad consideramos que vale $c=1$ (que elegimos la primera caja).
\todo[inline]{Esto ultimo no aporta nada, es solo para representar que podr\'ias estar eligiendo la primer caja? Yo lo sacaria si no tiene un objetivo claro}

\subsection{Principio de coherencia (regla del producto)} \label{sec:principio_choerencia} % (dados los datos de base emp\'irica)

El principio indiferencia (MaxEnt) nos permiti\'o definir la creencia conjunta inicial, y el principio de integridad (regla de la suma) nos permiti\'o derivar de ella la creencia marginal.
%
?`Pero c\'omo podemos preservar los acuerdos intersubjetivos cuando recibimos nueva informaci\'on?
\todo[inline]{Sacar todo esto de intersubjetivos! No esta definido nada de lo anterior en estos terminos, explicar en funcion de lo que se viene hablando}
%
Veamos un ejemplo.
% %
...
%
?`Cu\'al es la nueva creencia marginal sobre el regalo despu\'es de haber visto esta pista?
\todo[inline]{No entiendo nada, esto es lo que estaba en la secci\'on anterior... qu\'e cambia? Sinceramente no entiendo. Si es una reforma del ejemplo anterior, entonces traelo CLARAMENTE diciendo, nos paramos en la situaci\'on descripta por el modelo A o algo as\'i, dale contexto!!!! No podes pretender que el lector adivine.}
...
Por el principio de coherencia la nueva creencia que surge al haber visto un nuevo dato, debe ser la creencia previa que sigue siendo compatible con ese dato.
%
Para ello, nos quedamos con los caminos paralelos del modelo causal que siguen siendo compatibles con el observable, $P(r_i, s_2)$, dejando fijo el valor de $s=2$ y variable el valor $r=i$.
%
\begin{table}[ht!]
\centering
...
\begin{tabular}{|c|c|c|c||c|} \hline \setlength\tabcolsep{0.2cm}
       & \, $r=1$ \, &  \, $r=2$ \, & \, $r=3$ \, & \\ \hline
  $s=2$ & $1/6$ & $0$ & $1/6$ & $1/3$ \\ \hline
  \end{tabular}
%   \hspace{1.5cm}
  \begin{tabular}{|c|c|c|c||c|} \hline  \setlength\tabcolsep{0.2cm}
 & \, $r=1$ \, &  \, $r=2$ \, & \, $r=3$ \, & \\ \hline
  $s=2$ & $1/6$ & $0$ & $1/3$ & $1/2$ \\ \hline
  \end{tabular}
...
% %
La creencia que sobrevive, $P(r_i, s_2)$, no representa todav\'ia nuestra la creencia total pues $\sum_i P(r_i,s_2) = P(s_2) < 1$.
%
...
%
La nueva distribuci\'on de creencias a la que llegamos depende del modelo causal elegido.
%
\begin{figure}[ht!]
\centering
\begin{subfigure}[b]{0.48\textwidth}
...
\chapter{Discusi\'on 2. Los aprendizajes epist\'emicos y evolutivos}\label{ch:epistemico}
...
\subsection{Trabajo futuro: especificaci\'on y evaluaci\'on de argumentos causales.}
%
