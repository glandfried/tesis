...
Una de las técnicas más extendidas para evitar el costo computacional asociado a la evaluación completa del espacio de hipótesis consiste en seleccionar una única hipótesis en base a algún criterio ad-hoc, como por ejemplo maximizar la verosimilitud.
%
\begin{equation*}
...
Si bien esto permitió durante el siglo 20 ajustar funciones lineas a los datos, este enfoque introduce una serie de problemas.
%
Supongamos que tenemos un conjunto de datos generados a partir de una función objetivo sinoidal que desconocemos, y queremos elegir el modelo lineal que mejor la ajusta.
%
...
\begin{figure}[ht!]
  ...
  \end{subfigure}
  \caption{$\N(y \,| \, \text{sen}(x), \beta^2) \ \ x \in [-\pi,\pi]$ }
\end{figure}
...
%
En palabras de Shannon~\cite{shannon1948-theoryOfCommunication}, todo sistema de comunicación está compuesto por,
%
\begin{figure}[H]
    \centering
\tikz{
...
Por último, el mensaje enviado, la señal enviada y la señal recibida son producidas por la \textbf{realidad causal}.
%
Y el mensaje recibido es la \textbf{inferencia causal}, $P(\text{Indicador}|h, \text{M})$ \\
...
\subsection{Tasa de información}
