...
Al haber producido un modelo altenativo, apareció por primera vez la pregunta respecto de cómo evaluar correctamente modelos alternativos sobre los datos reales.
%
Este punto lo discuitimos con mi co-director y fue una bizagra para mi doctorado.
%
...
Hemos visto en la sección metodología que la evaluación de modelos aplicando estrictamente las reglas de la probabilidad nos obliga a commputar el posterior de los modelos $M$ dado los datos $D$, $P(M|D)$.
%
...
%
Hemos visto en la sección metodología que la forma correcta de evaluar modelos (a diferencia de las métricas ad-hoc) penalizan naturlmente la complejidad si ella no mejora la predección conjunta.
%
...
%
Sin embargo, luego de confirmar que mi modelo no era mejor en desempeño, y sin apoyo de la dirección en el trabajo, las estimaciones realizadas no fueron analizadas en detalle y la hipótesis original que había motivado todo este trabajo no alcanzó a ser respodida por la positiva o la negativa. 
%
...
%
Por ejemplo, en ATP pudimos ver que la complejidad añadida reducía la capacidad predicitva en jugadores con habilidades similar en los diferenstes pisos, como el caso Djokovic, y aumentaba la capacidad predictiva en jugadores con habilidades que varían según el piso, como el caso de Nadal.
%
...
%
En cualquier caso, nuestro modelo, al igual que TrueSkill tiene una dificultad para distinguir de dónde proviene el desepeño cuando dos personas siempre juegan juntas, por lo que el análisis hubiera quedado limitado a pares de jugadores no estables, y la hipótesis incicial no hubiera podido responderse. 
...
%
Debido a que no estaba implementado en ningún lenguaje, repliqué la implemtación en Python, Julia y R.
%
...
%
Pero fue mi formación epistemológica previa la que me permitió dar algunos saltos conceptuales claves a través de la lectura de algunos autores fundamentales.
...
%
En aquel momento comencé leyendo el excelente libro frecuntista ``An Introduction to Statistical Learning'' de Gareth James, Daniela Witten, Trevor Hastie y Robert Tibshirani~\cite{james2013-introductionToStatisticalLearning}.
%
Además de interesarme por los detalles técnicos relativos a cada algoritmo, me atraían las conclusiones epistemológicas que emergían por si solas de la actividad de apendizaje automático.
%
...
¿Pero qué tienen de especial las reglas de la probabilidad?
%
El segundo libro bayesiano que tuvo una enorme incluencia en términos conceptuales fue ``Probability Theory: The Logic of Science'' de Edwin Jaynes~\cite{Jaynes2003}.
%
En primer lugar, este libro comienza con el teorema de Cox, en el que las reglas de la probabilidad se derivan de una axiomatización básica del sentido común.
%
Pero más allá de eso, Jaynes generaliza el antiguo princpio de indiferencia, usando por Bernoulli y Laplace, por el principio de máxima entropía.
%
\begin{quotation}
...
%
Las distribuciones de creencias que cumplen con ausencia de mentira las podemos llamar ``verdades empíricas'', debido a que son proposiciones válidas universalmente (no afirman más de lo que saben sin dejar decir todo lo que sí saben).
%
Estas es la respuesta epistemológica fundamental que encontré durante el doctorado en computación.
%
Lo interesante es que es al mismo tiempo una respuesta metodológica.
%
Los princpios de integridad (no perder ni crear crencias espontanemanmete) y de coherencia (preservar la creencia previa compatible con la nueva información) inducen las reglas de la probabilidad que nos permiten preservar los acuerdos intersubjetivos a medida que se incorpora nueva información.

% Parrafo

Una lectura clave de esta étapa doctoral fueron los artículo científicos ``The ergodicity problem in economics'' de Ole Peters del año 2019~\cite{peters2019-ergodicityEconomics} y ``A new interpretation of information rate'' de John Kelly del año 1956~\cite{kelly1956}.
%
Ellos me permitieron entender


% Parrafo

Las últimas lecturas clave de mi etapa doctoral fueron sobre teoría de la información: el libro ``Information theory, inference and learning algorithms'' de David MacKay~\cite{mackay2003-informationInferenceLearning}.
...
Allí pudimos ver que aplicando estrictamente las reglas de


\section{El enfoque bayesiano en América Latina} \label{sec:bayesplurinacional}

\subsection{La brecha entre el potencial y lo real}
...
\subsection{Organización de la comunidad Bayes Plurinacional}
...
\subsection{El Congreso Bayesiano Plurinacional}
...
\section{Trabajo futuro.} \label{sec:volverAlFuturo}
...
\centering
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=1,width=\textwidth]{static/truesynergy/F2(published).pdf}
  \caption{\scriptsize error = $\frac{8}{2}\frac{1}{2} = 2$}\label{F2}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/F4(published).pdf}
  \caption{\scriptsize error = $\frac{8}{2}\frac{2}{3} = 2.67$}\label{F4}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/F5(published).pdf}
  \caption{\scriptsize error = $\frac{8}{2}\frac{4}{5} = 3.2$}\label{F5}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=1,width=\textwidth]{static/truesynergy/F6(published).pdf}
  \caption{\scriptsize error = $\frac{8}{2}\frac{9}{10} = 3.6$}\label{F6}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=1,width=\textwidth]{static/truesynergy/F7(published).pdf}
  \caption{\scriptsize error = $\frac{8}{3}\frac{1}{2} = 1.33$}\label{F7}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=1,width=\textwidth]{static/truesynergy/F8(published).pdf}
...
\centering
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=3,width=\textwidth]{static/truesynergy/H4(published).pdf}
  \caption{\scriptsize 75\% individuales}\label{H4_mu}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=3,width=\textwidth]{static/truesynergy/H1(published).pdf}
  \caption{\scriptsize 50\% individuales}\label{H1_mu}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=3,width=\textwidth]{static/truesynergy/H3(published).pdf}
  \caption{\scriptsize 25\% individuales}\label{H3_mu}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/H4(published).pdf}
  \caption{\scriptsize 75\% individuales}\label{H4_lambda}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/H1(published).pdf}
  \caption{\scriptsize 50\% individuales}\label{H1_lambda}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/H3(published).pdf}
...
\centering
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=3,width=\textwidth]{static/truesynergy/H5(published).pdf}
  \caption{\scriptsize 0\% individuales}\label{H5_mu}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/H5(published).pdf}
  \caption{\scriptsize 0\% individuales}\label{H5_lambda}
  \end{subfigure}\\
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=3,width=\textwidth]{static/truesynergy/H6(published).pdf}
  \caption{\scriptsize 5\% individuales}\label{H6_mu}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[page=2,width=\textwidth]{static/truesynergy/H6(published).pdf}
