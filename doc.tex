...
%
Por ejemplo, los cantos de las aves, sus estrategias de búsqueda de alimento, e incluso las rutas migratorias son comportamientos que se transmiten por v\'ia social, produciendo la formaci\'on de culturas locales~\cite{aplin2022-birdsCulture}.
%
...
El fundador de la teor\'ia de la probabilidad moderna, Pierre-Simon Laplace (1749-1827), parec\'ia tener cierta convicci\'on de que no exist\'ian eventos aleatorios en la naturaleza.
%
...
%
Dado $r=3.99$ y $\text{pop}_0 = 0.5$, la poblaci\'on tiene un comportamiento denominado \emph{ca\'otico}, en el que no se observan per\'iodos de ninguna longitud, pues el sistema produce trayectorias que nunca pasan por los mismos puntos (figura~\ref{fig:poblacion_caotico}).
%
\begin{figure}[ht!]
    \centering
    ...
    \end{subfigure}
    \caption{Tres comportamientos t\'ipicos del modelo de crecimiento poblacional. En todos los casos se mantiene la poblaci\'on inicial (pop$_0$) y se var\'ia \'unicamente el coeficiente de reproducci\'on de la poblaci\'on.}
    \label{fig:poblacion}
\end{figure}
%
Si modificamos levemente el valor inicial de la poblaci\'on bajo un comportamiento ca\'otico (por ejemplo de $0.5$ a $0.499$) obtenemos trayectorias que r\'apidamente divergen entre s\'i, siendo completamente diferentes luego de una decena de pasos temporales (figura~\ref{fig:sensibilidad}).
%
\begin{figure}[ht!]
    \centering
...
%
La incertidumbre asociada a eventos aleatorios, que ocurrir\'ian sin raz\'on suficiente, se la conoce como \emph{ontol\'ogica}
...
\subsection{Principio de integridad (regla de la suma)}\label{sec:principio_integridad}
...
%
En la teor\'ia de la probabilidad, la regla de la suma garantiza el principio de integridad, esto es que no se produzcan p\'erdidas ni creaciones espont\'aneas de creencias cuando la distribuimos entre hip\'otesis mutuamente excluyentes (i.e.~si hip\'otesis que no pueden ocurrir simult\'aneamente).

%

Por ejemplo, supongamos que una persona escondi\'o una regalo detr\'as de una caja entre tres.
%
El regalo puede estar en la caja 1, $r_1$, en la caja 2, $r_2$, o en la caja 3 ,$r_3$.
%
Luego, nuestro espacio de hip\'otesis es $R = \{r_1, r_2, r_3\}$ (Notaci\'on: $r_i \equiv r = i$, ``el regalo $r$ se encuentra en al posici\'on $i$'').
%
Por el principio de integridad, la creencia asignada a dos hip\'otesis $P(r_1 \, \cup \, r_2)$ debe ser la suma de creencias asignadas a cada una de ellas $P(r_1) + P(r_2)$.
%
Si además el espacio de hip\'otesis es exhaustivo la suma debe ser 1, pues estaremos sumando la creencia que asignamos a todas las posibles hip\'otesis.
%
...
% %
% Las ciencias emp\'iricas, desde la f\'isica hasta las ciencias sociales, deben validar sus proposiciones en sistemas abiertos que contienen siempre algún grado de incertidumbre.
%
...
S\'i.
%
En pocas palabras debemos evitar mentir: no afirmar más de lo que se sabe, sin dejar de decir todo lo que s\'i se sabe.
%
Este principio se formaliz\'o a mediados del siglo 20 como ``m\'axima entrop\'ia dadas restricciones'' o MaxEnt~\cite{Jaynes2003}.

% Parrafo

Antes de su definici\'on formal, esta idea ya era discutida en el siglo 18 por los fundadores de la teor\'ia de la probabilidad bajo el nombre de \emph{principio de indiferencia}~\cite{jaynes1984-bayesianBackground}.
%
Según este principio, cuando no existe razones para tener preferencia por ninguna de las opciones debemos asignar la misma creencia a todos los eventos para no mentir (MaxEnt).
%
...
%
En este ejemplo tenemos tres variables: la posición del regalo, $r$; la caja que elegimos, $c$; y la pista que recibimos $s$.
%
Por la descripción del juego sabemos que hay al menos una relaci\'on causal entre la posici\'on del regalo $r$ y la pista $s$, pues la pista no puede abrir la misma posici\'on en la que encuentra el regalo $s \neq r$.
%
Este modelo causal se especifica de forma gráfica en la figura~\ref{fig:modelo_causal_1}.
%
Además, podemos sospechar que existe una segunda relaciones causal, según la cual la pista tampoco está habilitada a abrir la caja que elegimos $s \neq c$ (figura \ref{fig:modelo_causal_2}).
...
\begin{figure}[ht!]
  ...
  \end{subfigure}
  \caption{Modelos causales alternativos. Modelo~\subref{fig:modelo_causal_1}: la pista s\'olo depende de la posici\'on del regalo. Modelo~\subref{fig:modelo_causal_2}: la pista depende de la posici\'on del regalo y la caja elegida previamente.
...
Ahora, para determinar la distribuci\'on de creencias honestas debemos maximizar la incertidumbre incorporando la informaci\'on que proveen los modelos causales.
%
Cada uno de los modelos causales alternativos restringe un conjunto de universos posibles.
%
Aplicando el principio indiferencia, dividimos la creencia en partes iguales en cada una de las bifurcaciones de los universos paralelos que producen los modelos causales.
%
En la figura~\ref{fig:caminos} mostramos cómo se divide la creencia a través de los universos paralelos de ambos modelos causales.
%
Los caminos que van de la raíz a la hoja representan universos posibles del modelo causal.
%
El estado del universo se representa con círculos.
%
La raíz contiene toda la creencia, la cual se divide en cada una de las bifurcaciones hasta llegar a las hojas.
%
De esta forma se maximiza la incertidumbre dada la información provista por los modelos causales.
%
Para simplificar el problema, consideramos que la variable $c$ es conocida, y vale $c=1$.
...
\end{subfigure}
\caption{Los posibles universos dado los modelos causales alternativos. Por el principio de indiferencia dividimos la creencia en partes iguales en cada una de las bifurcaciones de los caminos paralelos del modelo causal. Por simplicidad suponemos que $c=1$.}
\label{fig:caminos}
\end{figure}
%
En el modelo A, por cada posible posici\'on del regalo, se abren otros dos universos paralelos que se corresponden con las cajas hacia las que puede se\~nalar la pista, en las que no est\'a el regalo en ese universo (figura~\ref{fig:caminos_pre_montyhall}).
%
En cambio, en el modelo B, la pista genera dos universos solo cuando el regalo est\'a en la misma caja que elegimos, $r=c=1$ (figura~\ref{fig:caminos_montyhall}).
%
La creencia que llega a las hojas representa la \emph{creencia conjunta} honesta de que el regalo se encuentre en la caja $i$ y la pista se\~nale la caja $j$, $P(r=i, s=j|\text{Modelo})$.
%
\begin{table}[ht!]
\centering
...
\end{table}
%
Esta informaci\'on la podemos resumir en forma de tabla que representa la creencia asignada a cada uno de los universos paralelos (tabla~\ref{tab:creencia_conjunta}).
...
En la tabla~\ref{tab:creencia_conjunta} tenemos la creencia conjunta de que ocurren ambas variables a la vez.
%
Por el principio de integridad (o regla de la suma), la creencia de una variable individual (creencia marginal) no es más que la suma de las creencias asignadas a todos los universos en los que esa variable est\'a presente.
%
...
\end{figure}
%
La cerradura representa la variable $c$ (la elección de la caja) que por simplicidad consideramos que vale $c=1$ (que elegimos la primera caja).

\subsection{Principio de coherencia (regla del producto)} \label{sec:principio_choerencia} % (dados los datos de base emp\'irica)

El principio indiferencia (MaxEnt) nos permiti\'o definir la creencia conjunta honesta inicial, y el principio de integridad (regla de la suma) nos permiti\'o derivar de ella la creencia marginal honesta.
%
?`Pero c\'omo podemos preservar los acuerdos intersubjetivos cuando recibimos nueva informaci\'on?
%
Veamos un ejemplo.
% %
Supongamos que alguien nos indica con certeza que el regalo no est\'a en la caja del medio, $s=2$, después de haber elegido la primera caja, $c=1$.
%
...
Por el principio de coherencia la nueva creencia honesta, despu\'es de haber visto un nuevo dato, debe ser la creencia honesta previa que sigue siendo compatible con ese dato.
%
Para ello nos quedamos con los caminos paralelos del modelo causal que siguen siendo compatible con el observable, $P(r_i, s_2)$, dejando fijo el valor de $s=2$ y variable el valor $r=i$.
%
\begin{table}[ht!]
\centering
...
% %
La creencia que sobrevive, $P(r_i, s_2)$, no representa todavía nuestra la creencia total pues $\sum_i P(r_i,s_2) = P(s_2) < 1$.
%
Para que la creencia que sobrevive sea considerada nuestra nueva creencia total necesitamos normalizarla por la creencia total que sobrevive, de modo tal para que vuelva a sumar 1.
%
\begin{equation}
\underbrace{P(r_i| s_2, \text{Modelo})}_{\hfrac{\text{Nueva creencia}}{\text{}}} = \frac{\overbrace{P(r_i, s_2| \text{Modelo})}^{\hfrac{\text{Creencia que}}{\text{sobrevive}}}}{\underbrace{P(s_2| \text{Modelo})}_{\hfrac{\text{Creencia total}}{\text{que sobrevive}}}}
\end{equation}
%
Si multiplicamos a ambos lados por el denominador, obtenemos la expresión de la regla del producto de la teor\'ia de la probabilidad.
%
La nueva distribución de creencias a la que llegamos depende del modelo causal elegido.
%
\begin{figure}[ht!]
\centering
\begin{subfigure}[b]{0.48\textwidth}
...
%
La regla de la suma expresa el principio de integridad y establece que cualquier creencia marginal puede ser obtenida integrando la creencia conjunta.
%
\begin{equation}
P(r_i) = \sum_j P(r_i, s_j)
\end{equation}
%
La regla del producto expresa el principio de coherencia y establece que cualquier distribuci\'on conjunta puede ser expresada como el producto de distribuciones condicionales uni-dimensionales.
%
\begin{equation}
...
%
El sistema axiom\'atico de Kolmogorov~\cite{Kolmogorov1950} no busca justificar las reglas de la probabilidad pues la regla de la suma se incluye directamente como axioma y la regla del producto se propone como una definici\'on complementaria.
%
Este último sistema axiom\'atico es el m\'as utilizado por su simplicidad matemática y en su versi\'on original dice,
%
\begin{quotation}
...
\end{equation}
%
Los diferentes factores del teorema de Bayes han recibido nombres a lo largo de la historia, que revisamos a continuaci\'on.
...
%
Emergi\'o as\'i un nuevo sistema de informaci\'on, aut\'onomo del sistema de informaci\'on biol\'ogico, lo que nos permiti\'o ocupar todos los nichos ecol\'ogicos de la tierra como ningún otro vertebrado terrestre hab\'ia logrado antes~\cite{Boyd2011}.
...
%
Aprender a jugar al Go no solo implica adquirir una tradici\'on cultural milenaria, un conjunto de estrategias complejas acumuladas de generaci\'on en generaci\'on, sino también adaptarse a las innovaciones que surgen cada d\'ia.

% Parrafo

Aunque nuestra base de datos cuenta con más de 7 millones de jugadores y casi 15 millones de partidas entre noviembre de 2005 y julio de 2018, hemos decidido trabajar exclusivamente con los ocho primeros a\~nos (con \num{446000} jugadores y \num{818000} partidas) porque: 1. pudimos comprobar a través de la informaci\'on proporcionada por la plataforma OGS, que no hay bots en ese per\'iodo; 2. a finales de 2013 se produce una fusi\'on entre OGS y otro portal de Go que (Nova) que produce un cambio repentino en la estructura de la red.
%
...
%
Para calcular la posici\'on topol\'ogica de los individuos en la red, nos basamos en una metodolog\'ia clásica y robusta de ventanas deslizantes de análisis dinámico de grafos~\cite{Kossinets2006, Mcgregor2014}.
%
...
Según los principales hallazgos de la bibliograf\'ia sobre aprendizaje social y acumulaci\'on cultural~\cite{Derex2020}, el aprendizaje humano depende de al menos cuatro variables fundamentales: el \emph{entorno}, el \emph{experiencia individual}, el \emph{estructura de red} que da acceso a la informaci\'on cultural, y la \emph{habilidad inicial}.
%
\begin{figure}[ht!]
\centering \small
\tikz{
...
\caption{
  ...
  Los colores son los roles de las variables en el análisis estad\'istico: causa (rojo), efecto (rosa), control (gris), constantes (negro), no observables (blanco).
}
\label{fig:general_model}
\end{figure}
% Parrafo

En la figure~\ref{fig:general_model} resumimos el modelo causal en términos gráficos, donde las flechas representan relaciones causales entre las variables y los colores representan el rol que las variables cumplen en el análisis estad\'istico: causa (rojo), efecto (rosa), control (gris), constante (negro), no observable (blanco).
%
...
Esta secci\'on está dividida en cuatro partes: describimos el conjunto de datos; explicamos intuitivamente el modelo de estimaci\'on de habilidad; elaboramos la metodolog\'ia para estudiar los grafos en el tiempo; detallamos las medidas espec\'ificas que utilizamos para el análisis.
...
En este trabajo usamos métodos no experimentales, realizando análisis sobre datos de archivo, provistos por la plataforma de juego de Go en l\'inea OGS.
%
El conjunto de datos se ha hecho público hace varios a\~nos, y se encuentra disponibles para realizar trabajos de investigaci\'on cient\'ifica.
%
...
%
La plataforma OGS es, junto a KGS, la más utilizada para jugar al Go en l\'inea en el hemisferio occidental.
%
...
%
Sin embargo, el hecho de que la página oficial tenga la opci\'on de mostrar sus contenidos en casi $30$ idiomas indica que los participantes son de distintos pa\'ises alrededor del mundo.
...
%
Por otro lado, restringiendo el análisis al per\'iodo previo a 2013 garantizamos que la base de datos estuviera libre de bots, algo que pudimos constatar a través de consultas que le hicimos a la API que proveé OGS para interactuar con la base de datos.
%
...
Para analizar el aprendizaje en el tiempo, la mayor\'ia de los modelos utilizan el último posterior como prior del siguiente evento.
%
...
\subsection{Una metodolog\'ia clásica de análisis de grafos dinámicos}

Nos basamos en una metodolog\'ia clásica y robusta de grafos dinámicos~\cite{Kossinets2006, Mcgregor2014} para calcular la posici\'on topol\'ogica de los individuos en la red a lo largo del tiempo (donde los jugadores son los nodos y las partidas jugadas entre ellos son las aristas).
%
Esta técnica divide la red en peque\~nas subredes compuestas únicamente por nodos y vértices activos en un periodo determinado.
%
...
\begin{figure}[ht!]
	...
	\caption{
	Grado medio, tama\~no de la componente más grande,  largo promedio de los caminos m\'inimos y el coeficiente de clustering para ventanas de una semana, dos semanas y un mes.
	}
	\label{Figuras.DW}
\end{figure}
%
Siguiendo esta metodolog\'ia, en la figura~\ref{Figuras.DW} analizamos el grado medio, el tama\~no de la componente más grande, el largo promedio de los caminos m\'inimos, y el coeficiente de clustering, para diferentes tama\~nos de ventana: una semana, dos semanas, y un mes de largo.
%
...
%
En tercer lugar, el largo promedio de los caminos m\'inimos de la componente más grande muestra que, en general, los jugadores se encuentran a cuatro saltos de distancia.
%
Por último, el coeficiente de clustering muestra que la densidad del grafo (la proporci\'on de triángulos) es baja utilizando para cualquier tama\~no de ventana debido a que la plataforma estudiada favorece los v\'inculos sin necesidad que las personas tengan conocidos en común, evitando as\'i la emergencia de comunidades cerradas.
...
%
\item Las medidas de cercan\'ia, como \emph{closeness} y \emph{harmonic}, están basadas en la distancia t\'ipica que un nodo tiene hacia el resto: las personas que está cerca de todo el mundo tiene valores altos de cercan\'ia.
%
...
Llamamos indicadores de cercan\'ia a aquellos que están basados la distancia que los individuos tienen hacia el resto de los nodos.
%
La medida de cercan\'ia paradigmática es \emph{closeness}, definida como el rec\'iproco de las distancias m\'inimas promedio que un individuo tiene con todos los otros nodos.
%
...
%
La medida de intermediaci\'on paradigmática es \emph{betweenness}, definida como la suma de las proporciones de caminos m\'inimos entre todos los pares $q$ y $r$ que pasan por $p$.
%
\begin{equation}
...
%
El flujo que env\'ia un nodo $q$ hacia una nodo $r$ se transmite por los caminos m\'inimos, dividiendo el flujo en partes iguales cada vez que aparece más de una alterativa disponible.
%
\begin{equation}
	\emph{load}(p)= { \sum_{q,r} \text{Cantidad de flujo enviada por el nodo $q$ al $r$ que pasa por $p$} }
\end{equation}
%
Si hay un solo camino con distancia m\'inima, todo el flujo irá por ah\'i.
%
Estas dos soluciones están basadas en caminos m\'inimos.
...
%
En la subsecci\'on~\ref{sec:results_centralities} medimos la posici\'on topol\'ogica de los individuos en la red de intercambios de informaci\'on (indicadores de popularidad, cercan\'ia e intermediaci\'on) utilizando la metodolog\'ia clásica de análisis de grafos dinámicos~\cite{Kossinets2006, Mcgregor2014}.
%
...
%
Si bien la especificaci\'on de este modelo hab\'ia sido publicadas más de una década atrás~\cite{Dangauthier2007}, no exist\'ia ninguna versi\'on de c\'odigo abierto disponible hasta el lanzamiento de nuestros paquetes en \texttt{Julia}, \texttt{Python} y \texttt{R}~\cite{Landfried2021-TTT}.
%
...
\begin{figure}[ht!]
	...
	\end{subfigure}
	\caption{
	Distribuci\'on de medianas de las centralidades eigenvalue, harmonic y load, teniendo en cuenta únicamente las primeras 50 partidas jugadas de usuarios con al menos 150 partidas jugadas totales. Las l\'ineas punteadas muestran los percentiles 2, 40, 60 y 98.
	}
...
%
Observando el comportamiento de las distintas curvas de aprendizaje dado un valor de habilidad inicial, podemos permite comparar tasas de aprendizaje que según la experiencia individual deber\'ian ser iguales.
%
...
%
Y en cada columna se evalúa una misma medida de centralidad, una por cada tipo de indicador: popularidad (\emph{eigenvector}), cercan\'ia (\emph{harmonic}) e intermediaci\'on (\emph{load}).
%
...
%
El área sombrada representa dos desv\'ios estándar de la curva de aprendizaje estimada.
...
%
El impacto en las curvas de aprendizaje no es significativo solo en términos estad\'isticos, es además relevante en términos cualitativos en tanto la diferencia de habilidad entre tig
...
%
As\'i, aunque los jugadores expertos pueden entender mejor que nadie las estrategias de sus oponentes, al no tener mucho más que aprender no se observa por lo tanto el efecto del factor social en el aprendizaje.
...
%
En este contexto, decidimos seguir la metodolog\'ia clásica y robusta~\cite{Kossinets2006, Mcgregor2014} que estudia la dinámica del grafo utilizando ventanas deslizantes.
%
...
Nuestros resultados también están alineados con otros análisis nuestros anteriores, en el que observamos que repetir compa\~neros de equipo en juegos en l\'inea también mejora los procesos de aprendizaje a largo plazo a pesar de estar asociados con valores de centralidad bajos~\cite{Landfried2019}.
%
...
Por otro lado, la materia Teor\'ias Antropol\'ogicas Contempor\'aneas estaba a cargo, desde el inicio de la democracia, de un profesor que form\'o a un grupo de estudiantes en teor\'ias sistémicas y programaci\'on.
%
...
En mi tesis de licenciatura implement\'e un modelo de predicci\'on de alianzas~\cite{axelrod1993} y lo utilicé para analizar la historia de alianzas entre partidos pol\'iticos de varios gobiernos parlamentaristas.
%
...
%
En el a\~no 2022 creamos la comunidad Bayes Plurinacional con el objetivo de impulsar las comunidades bayesianas de la región.
%
\begin{figure}[ht!]
\centering
  \begin{subfigure}[b]{0.45\textwidth}
...
%
En la siguiente figura~\ref{fig:papers_bayesianos} se detalla la cantidad de artículos científicos vinculados al enfoque bayesiano publicados cada a\~no con al menos una autoría con filiación en América latina y el Caribe.
...
%
Se evalúan sistemas de comunicación con la realidad que difieren, no en los modelos causales, sino en los procedimientos que se usan para transmitir las se\~nales.
%
...
%
Discutimos el error de la teor\'ia de decisiones clásica, mostrando que no existe un dilema del prisionero bajo procesos como los epistémicos o evolutivos, que son de naturaleza multiplicativa.
%
