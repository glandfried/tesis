...
En este caso, el conjunto de cajas son el \emph{espacio de hipótesis}.
...
Dado este modelo, existe un conjuto finito de universos paralelos posibles.
%
Primero existen tres universos paralelos por cada posición que puede adquirir el regalo.
%
Y luego, dada la posición del regalo, existen dos unviersos paralelos por cada posición que puede adquirir la pista. 
%
Del mismo modo que aplicamos el principio de indiferencia al comienzo de la introducción, dividimos la creencia en partes iguales sobre las tres posibles posiciones del regalo.
%
...
\caption{}
\label{fig:modelo_pre_montyhall}
\end{figure}
%
...
%
\en{Here we offer the first software for \proglang{Julia}, \proglang{Python} and \proglang{R}, accompanied by an intuitive explanation for the general public and an in-depth explanation to people with basic knowledge in probability. }%
\es{Aquí ofrecemos el primer software para \proglang{Julia}, \proglang{Python} y \proglang{R}, acompañado por una explicación intuitiva comprensible para el público general, y otra explicación en profundidad específica para las persona con conocimientos básicos en probabilidad. }%
%%
...
\es{Todos los estimadores de habilidad ampliamente utilizados en la actualidad comparten alguna variante del modelo probabil\'istico propuesto por Elo~\citep{Glickman1999, Herbrich2007, VanDerLinden2016}. }%
%
\begin{figure}[ht!]
\centering \small
    \tikz{         
...
\en{The leading skill estimators used in the video game industry and academia, such as Elo, Glicko~\citep{Glickman1999}, TrueSkill~\citep{Herbrich2007} and IRT~\citep{VanDerLinden2016}, implemts a \emph{filtering} approach that propagate information incompletely, which prevents them from providing good initial estimates and ensuring comparability between estimates that are far away in time and space. }%
\es{Los principales estimadores de habilidad utilizados en la industria del video juego y la academia, como Elo, Glicko~\citep{Glickman1999}, TrueSkill \citep{Herbrich2007} e IRT \citep{Fox2010,VanDerLinden2016}, implementan un enfoque de \emph{filtrado} que propaga la información de manera incompleta, lo que les impide ofrecer buenas estimaciones iniciales y garantizar comparabilidad entre estimaciones alejadas en el tiempo y el espacio. }%
\en{The TrueSkill Through Time (TTT) model~\citep{Dangauthier2007} implemts a \emph{smoothing} approach that performs inference using a single Bayesian network that includes all historical events, allowing information to be propagated throughout the system, ensuring reliable initial estimates and comparability between distant estimates. }%
\es{El modelo TrueSkill Through Time (TTT)~\citep{Dangauthier2007} implementa una enfoque de \emph{smoothing} que realiza la inferencia usando una única red bayesiana que incluye todos los eventos históricos, lo que permite que la información se propague por todo el sistema, garantizando estimaciones iniciales fiables y comparabilidad entre estimaciones distantes. }%
%
...
\en{The main contribution of this work is the implementation of the state-of-the-art skill estimation model in the video game industry, that was not yet available in the programming languages with larger communities, such as \proglang{Julia}, \proglang{Python} and \proglang{R}. }%
\es{La principal contribución de este trabajo es la implementación del modelo de estimación de habilidades estado-del-arte en la industria de los videojuegos, que aún no estaba disponible en los lenguajes de programación con mayores comunidades, como \proglang{Julia}, \proglang{Python} y \proglang{R}. }%
%
...
%
\en{This information is not present either in the original articles \citep{Herbrich2007, Dangauthier2007} or in informal technical reports such as the one developed by \cite{Mosser2011}. }%
\es{Esta información no está presente ni en los artículos originales \citep{Herbrich2007, Dangauthier2007} ni en reportes técnicos informales como el desarrollado por \cite{Mosser2011}. }% 
%
...
%
\en{The level of improvement of this model is evident from its prior predictions, which are several orders of magnitude higher than those obtained with previous models~\cite{Tab:Models}. }%
\es{El nivel de mejora de este modelo se hace evidente a través de sus predicciones a priori, las cuales son varios ordenes de magnitud superiores a las obtenidas con los modelos anteriores~\cite{Tab:Models}. }%
%
...
%
\en{Being the state-of-the-art skill estimation model of the video game industry, we set ourselves the goal of developing the first official TrueSkill Through Time package in some of the programming languages with the largest communities such as \proglang{Julia}, \proglang{Python} y \proglang{R}. }%
\es{Al ser el modelo de estimación de habilidad estado-del-arte industria del video juego, nos propusimos el objetivo de desarrollar el primer paquete oficial de TrueSkill Through Time en alguno de los lenguajes de programación con mayores comunidades como \proglang{Julia}, \proglang{Python} y \proglang{R}. }%
%
...
%
\en{For example, it would be possible to use as observable data not only the ordinal values of the results (lose/tie/win), but also the score obtained by each team~\citep{Guo2012}. }%
\es{Por ejemplo, sería posible usar como datos observable no sólo los valores ordinales de los resultados (perder/empatar/ganar), sino la cantidad de puntos que obtuvo cada equipo~\citep{Guo2012}. }%
%
\en{Our tool could also include the socio-cultural factors that affect individual skills~\citep{VanDerLinden2016}. }%
\es{Nuestra herramienta también podría incluir los factores socio-cultural que afectan las habilidades individuales~\citep{VanDerLinden2016}. }%
%
...
% %
% \en{Special thanks to the authors of the original paper, Ralf Herbrich, Tom Minka, Thore Graepel, and Pierre Dangauthier \citep{Herbrich2007, Dangauthier2007}, a Heungsub Lee for having published the basic TrueSkill model in Python~\citep{lee-trueskill}, and Lucas Maystre for personally assisting us with the model comparison~\citep{Maystre2019}. }%
% \es{Agradecemos especialmente a los autores del artículo original Ralf Herbrich, Tom Minka, Thore Graepel, and Pierre Dangauthier \citep{Herbrich2007, Dangauthier2007}, a Heungsub Lee por haber publicado el modelo básico de TrueSkill en Python \citep{lee-trueskill}, y a Lucas Maystre por habernos ayudado personalmente a realizar la comparación de modelos~\citep{Maystre2019}.  }%
