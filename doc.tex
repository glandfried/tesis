...
El modelo causal de base emp\'irica utilizado por Elo para estimar la habilidad de los ajedrecistas profesionales en base al resultado de observado de una partida, supone que las habilidades (\emph{skills}) ocultas de dos agentes $i$ y $j$, $H_{S_i} = H_{S_j} = (-\infty,\infty)$, generan el resultado observable $H_R= \{\text{Ganar}, \text{Perder} \}$ a trav\'es de la diferencia $H_D = (-\infty,\infty)$ de los desempe\~nos aleatorios $H_{P_i} = H_{P_j} = (-\infty,\infty)$.
%
...
Arpad Elo propone a mediados del siglo 20 un procedimiento ad-hoc de selecci\'on de las hip\'otesis de habilidad basado en las predicciones del modelo (continuamos con la versi\'on provista en el art\'iculo~\cite{Herbrich2007}).
%
...
%
Sea el resultado $r \in H_R$, la ``sorpresa'' la define como,
%
