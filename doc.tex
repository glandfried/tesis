...
%
A los dos años de empezada la tesis pude verificar, realizando un estudio de antecedentes y consultando personalmente a expertos del área, que todavía no había madurado lo suficiente un área de intelgencia artificial distribuida capaz de producir sistema culturales complejos.
%
...
%
Desde aquel momento hasta ahora la vida adquirió una extraordinaria complejidad, tanto a nivel de cooperación como de especialización.
...
%
Algunas de las transiciones paradigmáticas son: de las células procariotas a las eucariotas; de los protozoa a los animales, plantas y hongos (diferenciación celular y emergencia de organiismos multicelulares); y de los individuos solitarios a las sociedades.
...
¿Cómo se explica esta tendencia permanente de la vida en favor de la cooperación y la especialización?
...
%
Siguiendo la idea de ese trabajo, y para simplificar el problema, vamos a considerar que el tamaño de la población de describe con un número real positivo y no como un número natural.
%
Debido a que los procesos de selección multiplicativos aparecen más de una vez a lo largo de esta tesis, vamos a remplazar desde ahora la palabra ``población'' por ``recurso'' (sea materia, energía o información).
...
\subsection{La ventaja de la diversificación}
...
\subsection{La ventaja de la especialización cooperativa}
...
\chapter{La lógica de las ciencias empíricas}


La ciencia es una institución humana con pretensión de cverdad, esto es de formular proposiciones que valgan ``universalmente'' para todas las personas.
%
...
% %
%Preferimos el concepto de ``acuerdos intersubjetivos'', por sobre el de ``objetividad'', porque el primero connota una actividad constructiva entre los sujetos, mientras que el segundo connota la imposici\'on directa del objeto sobre los sujetos.
%
...
%
La imposibilidad de crear números aleatrorios está incluida en la noción misma de ``método efectivo'', que se construye sobre el coonceptp de función.
...
\chapter{Primera base empírica: TrueSkill}
...
\section{Estimadores de habilidad}
...
\subsection{Elo. Una hip\'otesis indiciadora ad-hoc exitosa}
...
\subsection{TrueSkill: La hip\'otesis indiciadora universal}
...
%
%
% The TrueSkill method~\cite{herbrich2006-trueskill} was introduced in 2006 by Ralf Herbrich and has two patents~\cite{trueskill_patent_06,trueskill_patent_09}.
% TrueSkill shares the dependency model of the Elo rating system between skill, performance, and probability of winning.
% It extends it through a Bayesian model that incorporates a belief distribution of skills (prior), a model of team performance and a non-arbitrary update function (the posterior distribution).
%
% \subsubsubsection{Skill}
%
% \noindent{}One of the novelties of TrueSkill is the notion of the uncertainty of the skill estimation.
% The skill estimate $s_i$, previously represented by a scalar, is now represented as a prior distribution of beliefs with normal density function.
% \begin{equation}
% s_i \sim N(\mu_i, \sigma_i^2)
% \end{equation}
% where $\mu$ and $\sigma$ initially acquire arbitrary values.
%
% This is not an issue.
% What matters about the average is not its absolute value but the difference with other players.
% On the other hand, the standard deviation should be large enough to represent the uncertainty that we actually have with respect to the average.
% In general, $ \mu = 25 $ and $ \sigma=\frac{25}{3} $ are used as initial values.
%
% \subsubsubsection{Performance}
%
% \noindent{}As in the Elo system, it is assumed that the final outcome of the game depends on the $p_i$ performance of the players,
% \begin{equation}
% p_i \sim N(s_i, \beta^2)
% \end{equation}
%
% Now the probability of a given performance $p_i$ is defined as,
...
% \begin{figure}[ht!]
% \centering
% \includegraphics[width=.49\linewidth]{figures/Fig8}
% \caption{\small Performance distributions, $N(p_i \, ; \, s_i, \beta^2)$ weighted by the probability of the skill belief distribution $N(s_i \, ; \, \mu_i, \sigma_i^2)$. The area under the solid line must be integrated to compute a certain probability $p_i$.}
% \label{paso_1_multiplicacion_normales}
% \end{figure}
%
% Then, we can compute the probability of a given performance, $p_i$, by integrating the area under the solid line in Fig.~\ref{paso_1_multiplicacion_normales}.
% We rewrite the integral~\ref{p.p_i} using the symmetry property, $ N (x; \mu, \sigma^2) = N (\mu; x, \sigma^2)$.
% \begin{equation}
...
% The empirical probability distribution of individual and team games are exactly the same based on a Kolmogorov-Smirnov test, showing that the skill estimated by simple addition preserves the probability of winning based on this measure of team skill (Fig.~A in S1 File).
%
% Mathematically, a team's performance with two players we can see graphically in Fig.~\ref{paso_3_suma_normales_image}.
% \begin{figure}[ht!]
% \centering
% \includegraphics[width=.49\linewidth]{figures/Fig9}
% \caption{\small Joint probability of the performance of two teammates $i$, $j$. Lines parallel to the diagonal $ t_e = c $ represent team performance isobars.}
% \label{paso_3_suma_normales_image}
% \end{figure}
% To compute the probability of a given team's performance $c$ we must integrate the area under the corresponding isobar, $ t_e = c $ (See Fig.~\ref{paso_3_suma_normales_image}).
%
% It can be shown in a team with 2 members (S2 File),
% \begin{equation}
% \begin{split}
...
% \end{equation}
%
% It can be shown by induction (S2 File) that in a team with $n$ members the probability of a given team's performance is:
% \begin{equation}
% P(t_e|A_e) = N\left(t\,;\,\sum_{i\in A_e} \mu_i,\sum_{i\in A_e} \beta^2 + \sigma_i^2\right)
% \end{equation}
%
% \subsubsubsection{Difference}
%
...
% \end{equation}
%
% The observed outcome of a game it is modeled with an ordered vector of teams, $o$, such that $t_{o_1}< \dots < t_{o_{|A|}}$.
%
% \subsubsubsection{Posterior}
% \noindent{}In summary, the TrueSkill model can be represented by a graphical network (Fig.~\ref{modelo_trueskill}).
%
% \begin{figure}[ht!]
% \centering
% \includegraphics[width=1\linewidth]{figures/Fig10}
...
% \begin{equation}
%   P(\bm{s} \mid \bm{o}, A) =  \frac{P(\bm{o} \mid \bm{s}, A)  P(\bm{s})}{ P( \bm{o} \mid A)}
% \end{equation}
%
% The exact posterior could be computed by performing the sum-product algorithm~\cite{kschischang2001-factorGraphsAndTheSumProductAlgorithm} over the factor graph (Fig.~\ref{bayes_factor_network}).
%
% \begin{figure}[ht!]
% \centering
% \includegraphics[width=1\linewidth]{figures/Fig11}
% \caption{Bayes factor network of TrueSkill method.}
% \label{bayes_factor_network}
% \end{figure}
%
% With two teams, the exact posterior update function is (See all the details S2 File),
%
% \begin{equation}
...
%  \end{cases}
% \end{equation}
% where $\delta(s_i) = \delta - \mu_i + s_i$, the expected difference between teams replacing the estimated skill ($\mu$) by their true skill $s_i$ (Fig.~\ref{posterior_ganador} and~\ref{posterior_perdedor}).
%
% \begin{figure}[ht!]
% \centering
...
\chapter{Base empírica 2. TrueSkill Through Time: el estimador de habilidad estado del arte} \label{ch:ttt}
...
%
La soluci\'on metodol\'ogica de Elo es simple y astuta: actualizar las estimaciones puntuales previas, $s_{i_\text{old}}$ y $s_{j_\text{old}}$, en base a la sorpresa, i.e.~el complemento de la predicci\'on del resultado observado.
%
\begin{equation} \label{eq:elo_delta}
 \Delta = \underbrace{\left(1-P(\,r\,|\,s_{i_\text{old}},s_{j_\text{old}}\,)\right)}_{\text{Sorpresa}}
\end{equation}
%
...
Para corregir los problemas del algoritmo Elo es necesario computar la incertidumbre de las estimaciones.
%
...
%
El \ref{eq:bayes_theorem} actualiza de forma \'optima las creencia sobre las hip\'otesis, maximizando la incertidumbre luego de haber incorporar la informaci\'on provista por el modelo y los datos.
%
...
%
\begin{equation}\label{eq:event_inference}
 \underbrace{p(\overbrace{\text{Habilidad$_i$}}^{\text{\en{Hidden}\es{Oculta}}}|\overbrace{\text{Result\es{ado}}}^{\text{Observ\en{ed}\es{ado}}}, \text{Model\es{o}})}_{\text{Posterior}} = \frac{\overbrace{P(\,\text{Result\es{ado}}\,|\,\text{\en{Skill}\es{Habilidad}$_i$}\,,\text{Model\es{o}})}^{\text{\en{Likelihood}\es{Verosimilitud}}}\overbrace{p(\text{\en{Skill}\es{Habilidad}$_i$})}^{\text{Prior}}}{\underbrace{P(\text{Result\es{ado}}\,|\,\text{Model\es{o}})}_{\text{Evidenc\en{e}\es{ia} o\en{r}\es{ predicci\'on a} prior \en{prediction}}}}
\end{equation}
%
...
%
Por lo tanto, la predicci\'on a priori del resultado observado, o evidencia, es la densidad acumulada ($\Phi$) de todos los valores positivos de la gaussiana de diferencia de desempeños (Equation~\ref{eq:evidence}).
%
A partir de ahora el rol del modelo se dejar\'a impl\'icito.
%
\begin{equation}\label{eq:evidence}
 \overbrace{P(r)}^{\text{Evidencia}} = 1-\Phi(0 \, | \overbrace{\mu_i^{\phantom{2}} - \mu_j}^{\hfrac{\text{\en{Expected}\es{Diferencia}}}{\text{\en{difference}\es{esperada}}}} , \, \overbrace{2\beta^2 + \sigma_i^2+ \sigma_j^2}^{\hfrac{\text{\en{Total}\es{Incertidumbre}}}{\text{\en{uncertainty}\es{total}}}})
\end{equation}
%
La evidencia es una predicci\'on hecha con todas las hip\'otesis a priori.
%
...
%
\begin{equation}\label{eq:posterior_win}
\underbrace{p(\,s_i\, | \, r \, )}_{\text{Posterior}} \propto \underbrace{1-\Phi(0 \, |  s_i - \mu_j , \, 2\beta^2 + \sigma_j^2)}_{\text{Verosimilitud} \ P(r|s_i)} \,  \underbrace{\N(s_i \, | \, \mu_i,\, \sigma_i^2)}_{\text{Prior} \ p(s_i)}
\end{equation}
%
Donde el posterior normalizado se obtiene dividiendo el lado derecho con la evidencia, $P(r)$.
%
...
\begin{figure}[ht!]
    \centering
    \includegraphics[page={2},width=.6\linewidth]{figures/posterior_win}
    \caption{
    %
...
\section{Aplicaciones} \label{sec:illustrations}
...
\chapter{Implementación del estimador de habilidad estado-del-arte}
...
\subsection{La soluci\'on exacta para eventos con dos equipos}\label{sec:2vs2}
%
