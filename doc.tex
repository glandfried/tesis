...
%
%Por ello, a lo largo de los siglos se fueron desarrollando t\'ecnicas para resolver este problema (f)
%
% El primer ciclo de soluciones llega de la mano de f\'isica estad\'istica, en la segunda mitad del siglo 19~\cite{jaynes1957-informationTheoryAndStatisticalMechanics, Jaynes2003}.
% %
...
% %
% Por ello, antes de presentar los modelos b\'asicos de estimaci\'on de habilidad,
%
% En la sección presentamos las reglas de la probabilidad (secc\'on \ref{sec:reglas_de_la_probabilidad}), y discutimos revisamos los fundamentos epistemol\'ogicos de la construcci\'on de conocimiento en contextos de incertidumbre, y
%
% % Parrafo
%
...
%
Esta idea parece estar relacionada con la imposibilidad de \emph{crear azar}, es decir de crear un algoritmo capaz de generar a veces un resultado y a veces otro dadas exactamente las mismas condiciones iniciales.
%
...
%
La imposibilidad de crear n\'umeros aleatorios est\'a determinada por la definici\'on misma de funci\'on, $f(x) = y$, pues las mismas condiciones iniciales $x$, luego de aplicado un algoritmo $f$, debe producir siempre el mismo resultado $y$.

% Parrafo

El fundador de la teor\'ia de la probabilidad moderna, Pierre-Simon Laplace (1749-1827), parecía tener cierta convicci\'on de que no exist\'ian eventos aleatorios en la naturaleza.
%
...
% %
Revisemos un sistema determinista sencillo.
%
Supongamos que el tamaño de una población, medida como proporci\'on de una capacidad m\'axima, se actualiza exclusivamente en función del su tama\~no previo y de un factor de reproducci\'on $r$,
%
% Parrafo
% \begin{figure}[H]
%
...
%
%     \node[latent, right=of n1, xshift=3cm, minimum size=1.25cm] (n2) {$\text{pob}_{t+\Delta t}$} ;
%
%      \path[->] (n1) edge node [yshift=0.5cm] {} (n2);%{$r \cdot \text{pob}_t \cdot (1-\text{pob}_t)$}
%
% }
% % \caption{Modelo causal expresado gr\'aficamente. La flecha representa la relaci\'on causal entre las variables.
% % \label{fig:modelo_poblacional}
% \end{figure}
%
\begin{align*}
 \text{Poblaci\'on}&(0) = \text{pop}_0 \\
 \text{Poblaci\'on}&(t+\Delta t) = r \cdot \text{Poblaci\'on}(t)\cdot (1-\text{Poblaci\'on}(t))
\end{align*}
con $r \in [0,4]$ y pop$_0 \in [0,1]$.
%
Determinados los valores iniciales $r$ y pop$_0$ queda determinado el valor de la población en cualquier tiempo.
%
Dado $r=1.3$ y $\text{pop}_0 = 0.5$, la poblaci\'on tiende a un valor fijo  (figura~\ref{fig:poblacion_punto_fijo}).
%
Dado $r=3.5$ y $\text{pop}_0 = 0.5$, la poblaci\'on oscila en un per\'iodo (figura~\ref{fig:poblacion_periodico}).
%
Dado $r=3.5$ y $\text{pop}_0 = 0.5$, la poblaci\'on tiene un comportamiento denominado ``ca\'otico'', en el que no se observan per\'iodos de ninguna longitud, pues el sistema produce trayectorias que nunca pasan por los mismos puntos (figura~\ref{fig:poblacion_caotico}).
%
\begin{figure}[ht!]
    \centering
    ...
    \end{subfigure}
    \caption{Tipos de comportamientos del modelo determinista.}
    \label{fig:poblacion}
\end{figure}
%
Si modificamos levemente el valor inicial de la población bajo un comportamiento caótico, de $0.5$ a $0.499$, produce trayectoria que r\'apidamente divergen hasta volverse completamente diferentes (figura~\ref{fig:sensibilidad}).
%
\begin{figure}[ht!]
    \centering
    ...
    \end{subfigure}
    \caption{Sensibilidad a condiciones iniciales: $\text{pop}_0 = 0.5$ (negro), $\text{pop}_0 = 0.499$ (rojo).}
    \label{fig:sensibilidad}
\end{figure}
%
Esta sensibilidad a las condiciones iniciales de modelos deterministas impone, en los hechos, límites a la capacidad predictiva.

%



Alcanza con no conocer las condiciones iniciales con precisión \emph{infinitesimal} para introducir ignorancia completa en sistemas deterministas.
%
A esta incertidumbre, que proviene de nuestra ignorancia, se la conoce como \emph{epistémica}.

%

Si la naturaleza, por algún motivo no fuera determiniasta

existe otro concepto de incertidumbre que se la conoce como \emph{ontológica}
