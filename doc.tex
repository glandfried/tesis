...
El problema hist\'orico para la aplicaci\'on estricta de las reglas de la probabilidad ha sido el costo computacional asociado a la evaluaci\'on completa del las hip\'otesis de investigación (o experimento) $\mathcal{H} = \{H_1, H_2, \dots, H_N\} $.
%
Con hip\'otesis binarias $|H_i|=2$, el tama\~no de la distribuci\'on conjunta crece de forma exponencial, $2^N-1$.
%
En hipótesis continuos, la imposibilidad matemática de resolver las integrales requeridas para la aplicación la regla de la suma ha impedido realizar la inferencia de las hipótesis activas al interior de cada modelo, y de evaluar correctamente el propio modelo.
%
El famoso modelo lineal es un caso paradigmático: es un modelo que tiene solución analítica pero que quedó impedido de ser implementado correctamente hasta el desarrollo de la computación moderna debido al costo computacional de la inversión de matrices.
...
%
En particular, un modelo causal lineal de grado 3 supone que el mundo tiene $3$ causas $W_1$, $W_2$, $W_3$, de $N+1$ consecuencias $Y_1$ a $Y_{N+1}$, con dependencias causales entre variables regida por la siguiente relación.
%
\begin{equation}
y_i \leftarrow w_0 + w_1 x_i + w_2 x_i^2
\end{equation}
%
La flecha representa la dependencia causal.
%
Esta notación, adoptada en el área de Inferencia Bayesiana Causal~\cite{pearl2009}, involucra la idea de que cambios en los valores de las causas producen cambios en los valores de las consecuencias, pero no a la inversa.
%
Las hipótesis de investigación (o experimento) del modelo causal lineal más básico ($N=1$) son $\mathcal{H}=\{Y_1, Y_2, W1, W_2, W_3 \}$, con $Y_1=Y_2=W_1=W_2=W_3=(-\infty, \infty)$.
%
...
%
El vectores $\bm{w}$ contienen las hipótesis elementales de $w_1$ a $w_D$, el vector $\bm{y}$ contienen las hipótesis elementales de $y_1$ a $y_N$, y el vector $\bm{x}$ contiene los valores $x_1$ a $x_N$.
%
