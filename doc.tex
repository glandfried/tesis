...
%
La teoría de juegos evolucionaria \cite{taylor1978-replicatorDynamic, maynardSmith1982-evolutionGameTheory, boyd1985-evolutionaryProcess} modela los equilibrios evolutivos que se producen entre estrategias en competencia, en el que la adaptabilidad de cada variante depende de las frecuencias de las otras variantes en la población.
%
...
%
El objeto de estudio que motivó el plan, entender el sistema de información cultural, puede ser visto como un sistemas natural de procesamiento de información distribuido.
%
Entender cómo cambia la cultura en el tiempo no solo es uno de los problemas fundamentales de la antropología, es también uno de los problemas que las ciencias de la computación responderá en los próximos años.
%
En efecto, al comenzar la tesis pude verificar, consultando personalmente a expertos del área, que todavía no había madurado lo suficiente la inteligencia artificial multi-agente.
%
Sin embargo, los desarrollos en esta área ocurren con gran velocidad.
%
...
% %
% En esa cosa concreta se juega la suerte de teorías enteras, y con ellas, del trabajo de generaciones de investigadores.
% %
...
%
Por un lado, evitamos sobrecargar el doctorado en computación de tareas de recolección de datos (una tarea en si misma en ciencias empíricas), que pudimos resolver creando sistemas computaciones para extraer, transformar y cargar los información disponible en servidores públicos, alimentando las bases de datos sobre las que trabajamos a lo largo de esta tesis.
%
...
%
En el transcurso de la tesis esta pregunta fue mutando hacia estudiar los factores sociales que afectan el aprendizaje individual esperado por el simple experiencia individual.
%
Como se discutirá más adelante, los mecanismos fundamentales que gobiernan el cambio cultural, según los modelos matemáticos, están relacionados a estrategias de aprendizaje social.
%
Imitar a la mayoría, imitar a la persona más prestigiosa, son diferentes estrategias de aprendizaje social, que afectan el comportamiento del sistema de información cultural.
...
%
La ciencia es una institución humana que tiene pretensión de verdad, esto es de alcanzar acuerdos intersubjetivos ``universalmente'', que valgan para todas las personas.
%
...
%
Sin embargo, las ciencias empíricas (desde la física hasta las ciencias sociales) deben validar sus proposiciones dentro de sistemas abiertos que contienen siempre algún grado de incertidumbre.
%
¿Es posible alcanzar acuerdos intersubjetivos (verdades) en contextos de incertidumbre?
%
Sí, evitando mentir: no decir más de lo que se sabe incorporando toda la información disponible.
%
...
% Parrafo: Resultados y estructura de la tesis.

Los resultados de esta tesis están ordenados siguiendo el recorrido temporal.
%
En la continuación de este capítulo se presenta la motivación y los objetivos de esta tesis.
%
...
%
Sin embargo, los procesos evolutivos de selección de las formas de vida, por secuencia de tasas de supervivencia y reproducción, son como los procesos de selección de las hipótesis en la teoría de la probabilidad, por secuencias de predicciones, de naturaleza multiplicativa.
%
...
\subsection{La función de costo epistémico-evolutiva}
...
\begin{equation*}
\overset{\hfrac{\text{\small \En{Sum rule}\Es{Regla de la suma}: }}{\phantom{x}}}{P(A) + P(\text{no\En{t} } A) = 1} \hspace{0.4cm} , \end{equation*}
...
Estas son las reglas de la probabilidad para actualizar las creencias en contextos de incertidumbre, no hay más.
%
...
%
(\subref{fig:modelo_causal}) Modelo causal en el cual la pista $s$ no puede señalar la caja elegida $c$ (con la cerradura) ni la caja donde se encuentra el regalo $r$ (la hipótesis oculta): $s\neq c$ y $s\neq r$.
%
(\subref{fig:caminos_montyhall_compatibles}) Antes de ver la pista, buscamos la creencia que maximiza la incertidumbre dado el modelo causal (dividiendo la creencia en partes iguales en cada bifurcación de los universos paralelos).
%
(\subref{fig:f3}) La nueva creencia no es más que la creencia inicial (tabla) que sigue siendo compatible con el dato $s_2$, expresada como 100\%
}
...
%
Por ejemplo, la predicción de un modelo, por la regla de la suma, es el promedio ponderado de las predicciones de todas las hipótesis que la componen.
%
\begin{equation*}
\begin{split}
...
%
Esto hace que los humanos tengamos como especie un elemento epistémico no presente en las variantes que produce la teoría de la probabilidad, que será de interés a lo largo de toda esta tesis.
...
\subsection{Propiedades de la función de costo epistémico-evolutiva}
...
%
Si nuestros recursos iniciales son $\omega_0$, los recursos que obtenemos con una apuestas $b \in [0,1]$, después de una Cara y una Seca, se actualiza como un producto de nuestros resultados (ver ecuación 1).
%
\vspace{-0.4cm}
\begin{figure}[ht!]
 \centering
...
\todo[inline]{Introducir ecuación cooperativa.}
...
%
En nuestro ejemplo, si en cada paso temporal los individuos redistribuyen los recursos en partes iguales antes de apostar, la tasa de crecimiento de todos los individuos se vuelve positiva.
...
%
En un grupo cooperativo de tamaño $N$, en el que los recursos se dividen en partes iguales, la tasa de crecimiento de los individuos pasa a ser el promedio de la recursos luego de que $m_c$ y $m_s$ individuos obtienen Cara y Seca respectivamente, con $m_c + m_s = N$ (ver ecuación de la figura~\ref{fig:esp}).
%
\begin{figure}[ht!]
\centering
 \begin{subfigure}[c]{0.50\textwidth}
...
\begin{flushright}
 ...
 \end{subfigure}
 \caption{
En la ecuación vemos la tasa de crecimiento en grupos cooperativos. En la figura vemos la tasa de crecimiento para grupos de tamaño 1 a 5 (colores), para todas las posibles apuestas (eje x). A medida que aumenta el tamaño del grupo, la apuesta óptima se va especializando hacia la opción mejor paga por la casa de apuesta. Cuando el tamaño del grupo tiende a infinito, la puesta óptima es apostar todo a la mejor opción
 }
 \label{fig:esp}
 \vspace{-0.1cm}
 \end{figure}
%
A medida que el grupo aumenta de tamaño, la apuesta óptima se va ``especializando'' en la mejor opción disponible (figura~\ref{fig:esp}).
...
%
En casos como estos, si bien la especialización sigue siendo valiosa para los grupos cooperativos, la variabilidad del proceso produce una ventaja adicional en favor de los grupos con mayor heterogeneidad interna.
%
Ésta es la \emph{propiedad ecológica}.
%
Si $h$ es la proporción de individuos que se encuentran en el hemisferio $A$, la tasa de crecimiento de los individuos en una grupo cooperativo especializado grande ($\lim_{N\rightarrow \infty} r(h)$) se maximiza cuando la distribución entre hemisferios es en partes iguales, con $h=0.5$ (figura~\ref{fig:divCoop}).
%
\begin{figure}[ht!]
\vspace{0cm}
\centering
...
\begin{flushright}
 ...
 \end{subfigure}
 \caption{
 En la ecuación vemos la tasa de crecimiento en función de la proporción en hemisferio A, $h$, de un grupo grande especializado en la mejor opción, en el que los individuos aplican la apuesta óptima correspondiente a la etapa del ciclo que experimentan.
 En la figura vemos que la tasa de crecimiento de los individuos del grupo se maximiza cuando la heterogeneidad entre individuos es máxima.
 }
 \label{fig:divCoop}
 \vspace{-0.1cm}
 \end{figure}
...
%
Sin embargo, bajo procesos de selección multiplicativa no se puede sacar provecho mediante la deserción porque, al aumentar las fluctuaciones los cooperadores de quienes se depende, aumenta también las propias fluctuaciones afectando negativamente la propia tasa de crecimiento de largo plazo.
%
En la figura~\ref{fig:desert} mostramos la tasa de crecimiento de individuos en poblaciones cooperativas con 0, 1 y 2 desertores.
\begin{figure}[ht!]
\centering
\hspace{-1cm}
 \begin{subfigure}[c]{0.40\textwidth} \centering

 \vspace{-0.3cm}

{\footnotesize \En{Prisoner's dilemma}\Es{Dilema del prisionero}}
 \begin{equation*}
...
$\hfrac{\text{\footnotesize \En{Unilateral defection is better}\Es{La deserción unilateral es}}}{\text{\footnotesize \En{than mutual cooperation}\Es{mejor que la mutua cooperación}}}$
  \end{subfigure}
  \ \ \ \ \ \
 \begin{subfigure}[c]{0.40\textwidth} \centering
  ...
  \end{subfigure}
 \caption{
En la ecuación vemos la matriz de pagos del dilema del prisionero. En la figura vemos la tasa de crecimiento de los individuos dentro de grupos cooperativos (colores) con 0, 1 y 2 desertores. Las curvas con mayor variabilidad son los individuos desertores, el resto son los cooperadores. Los desertores tienen una tasa de crecimiento menor que la tenían cuando cooperaban (\emph{propiedad mayor}), a pesar de que inicialmente pudieran obtener un beneficio (\emph{propiedad menor}).
 }
 \label{fig:desert}
 \end{figure}
...
\subsection{Coevolución genético-cultural}

\todo[inline]{Recordar las transiciones mayores en evolución y epistémicas}

\en{Moreover, in the history of human beings, epistemic cooperation (transmission of knowledge between individuals) allowed the emergence of worldviews, such as science and culture, where highly specialized paradigms in particular domains coexist.}%
\es{Además, en la historia del ser humano la cooperación epistémica (transmisión de conocimiento en entre individuos) permitió la emergencia de cosmovisiones, como la ciencia y la cultura, donde paradigmas altamente especializados en dominios particulares coexisten.}%
%
\en{The positive effects are clear.}%
\es{Los efectos positivos son evidentes.}%
%
\en{Before the cultural transition, we were in serious danger of extinction, as evidenced by the low diversity of the human genome, even compared to the closest hominins.}%
\es{Antes de la transición cultural, estábamos en grave peligro de extinción, lo que se evidencia en la baja diversidad del genoma humano, respecto incluso de los homininos más cercanos.}%
%
\en{But when knowledge, that previously had to be rediscovered individually, became a common resource passed on from generation to generation, we began to be able to occupy all the ecological niches of the earth as no other terrestrial vertebrate had done before.}%
\es{Pero cuando el conocimiento, que antes debía ser redescubierto individualmente, pasó a ser un recurso común transmitido de generación en generación, comenzamos a ser capaces de ocupar todos los nichos ecológicos de la tierra como ningún otro vertebrado terrestre lo había logrado antes.}%
%
\en{Organized in small nomadic societies, we managed to walk from Africa to South America (arrows in Figure~\ref{fig:poblamiento}).}%
\es{Organizados en pequeñas sociedades nómadas, logramos llegar caminando de África hasta sud América (flechas de la figura~\ref{fig:poblamiento}).}%
...
\centering
 \begin{subfigure}[b]{0.145\textwidth} \centering
  \includegraphics[width=\linewidth]{static/agriculturaR.pdf}
  \caption{}
...
\en{In turn, the ecological reciprocity through prolonged symbiotic interaction with plants and animals led to the independent emergence of agriculture in sub-Saharan Africa, Middle East, China, Oceania, North America and South America (red points in Figure~\ref{fig:poblamiento}).}%
\es{A su vez, la reciprocidad ecológica por interacción simbiótica prolongada con plantas y animales produjo la aparición independiente de la agricultura en África subsahariana, Oriente medio, China, Oceanía, América del Norte y América del Sur (puntos rojos de la figura~\ref{fig:poblamiento}).}%
%
\en{With it came an increase in population that intensified the processes of cumulative cultural evolution, making these regions the main technological centers of humankind.}%
\es{Con ella se produjo un aumento de la población que intensificó los procesos de evolución cultural acumulativa, haciendo de estas regiones los principales centros tecnológicos de la humanidad.}%

% Parrafo

\en{During the 1400s, prosperous societies flourished all over the world.}%
\es{Durante el año 1400 florecían sociedades prósperas en todo el mundo.}%
%
\en{The Pacific Ocean was fully occupied, and there had already been exchanges between Oceania and South America, which is reflected in the genetics of the current populations (figura~\ref{fig:pacifico}).}%
\es{El océano Pacífico estaba totalmente ocupado, y ya se habían producido intercambios entre Oceanía y América del Sur, que se refleja en la genética de las poblaciones actuales (figura~\ref{fig:pacifico}).}%
%
\en{In the garden of human genetic and cultural diversity, sub-Saharan Africa, the Bantu society developed, among others.}%
\es{En el jardín de la diversidad genética y cultural humana, África subsahariana, se desarrollaba entre otras, la sociedad Bantu.}%
%
\en{But China was the main productive and technological center of the world after 2 millennia.}%
\es{Pero era China el principal centro productivo y tecnológico del mundo después de 2 milenios.}%
%
\en{By 1400, the Arab world traded its products from the Atlantic Ocean in Spain to the Pacific Ocean in the Philippines.}%
\es{Para el año 1400, el mundo Árabe comerciaba sus productos desde el océano Atlántico en España, hasta el océano Pacífico en las Filipinas.}%
%
\en{After conducting the explorations led by Zheng He (1405-1433) around the world, China recognizes that there is nothing interesting beyond its borders.}%
\es{Luego de realizar las exploraciones dirigidas por Zheng He (1405-1433) alrededor del mundo, China reconoce que no hay nada interesante afuera de sus fronteras.}%

% Parrafo

\en{At the same time, the massive destruction of cultural diversity previously produced by the Roman Empire in its environment had led Western Europe, against the rest of the world, into a long process of cultural involution and internal violence known as the ``Middle Ages''.}%
\es{Al mismo tiempo, la masiva destrucción de la diversidad cultural producida previamente por el imperio Romano en su entorno, había conducido a Europa occidental, a contramano del resto del mundo, a un largo proceso de involución cultural y de violencia interna conocido como ``Edad media''.}%
%
\en{Breaking the cooperative pact had concrete negative consequences for this society.}%
\es{La ruptura del pacto cooperativo tuvo consecuencias negativas concretas para esta sociedad.}%
%
\en{At this stage, the criterion of authority as the foundation of ``authentic knowledge'' became commonplace within feudal society.}%
\es{En esta etapa, se generalizó al interior de la sociedad feudal el criterio de autoridad como fundamento del ``saber auténtico''.}
%
\en{A unbridled penal system was reborn from the so-called \emph{libris terribilis} and the institutions inherited from the Western Roman Empire began to regulate community relations of sexual reproduction in a more detailed way than with private property.}%
\es{Un sistema penal desenfrenado renació de los llamados \emph{libris terribilis} y las instituciones heredadas del imperio romano de occidente comenzaron a regular las relaciones comunitaria de reproducción sexual de forma más detallada que con la propiedad privada.}%
%
\en{The war against women is definitively formalized with the publication of the \emph{Malleus maleficarum} in 1484, which would be the second best seller after the Bible for the next 200 years.}%
\es{La guerra contra las mujeres se formaliza definitivamente con la publicación del \emph{Malleus maleficarum} en 1484, que será el segundo best seller después de la Biblia durante los siguientes 200 años.}%

% Parrafo

\en{Even there is an evolutionary disadvantage of defection, certain conjunctures can produce its emergence in the short term (\emph{minor property}).}%
\es{Aunque exista una desventaja evolutiva de la deserción, ciertas coyunturas pueden producir su emergencia en el corto plazo (\emph{propiedad menor}).}%
% Parrafo
\en{Thus, the coincidence of a set of events, triggered by the massive feudal migration to America, suddenly put this historically marginal society in a situation of global privilege.}%
\es{Así es que la coincidencia de un conjunto de eventos, que se desencadenaron por la masiva migración feudal a América, puso de repente a esa sociedad, históricamente marginal, en una situación de privilegio mundial.}%
%
\en{The diseases transmitted by the feudal explorers eliminated, at least, 2/3 of the American population.}%
\es{Las enfermedades transmitidas por los exploradores feudales eliminó, al menos, 2/3 de la población americana.}%
%
\en{However, there was no geopolitical shift until explorers discovered the silver mountain of Potosi in 1546, a metal that China had recently incorporated as one of its official currencies.}%
\es{Sin embargo, no se produce ningún giro geopolítico hasta que los exploradores descubren la montaña de plata de Potosí en 1546, metal que China había incorporado recientemente como una de sus monedas oficiales.}%
%
\en{Thanks to it, a quarter of a century later Western Europe finally managed to break its isolation (battle of Lepanto 1571) and begin the long cycle of importing foreign technology, mainly from China.}%
\es{Gracias a ella, un cuarto de siglo después Europa occidental finalmente logra romper su aislamiento (batalla de Lepanto 1571) y dar inicio a un largo ciclo de importación de tecnología extranjera, principalmente China.}%

% Parrafo
%
% \en{It is only after feudal society acquires a privileged position in the world system that a new debate about the sources of validation of knowledge begins within this society.}%
% \es{Sólo después de que la sociedad feudal adquiere una posición de privilegio en el sistema mundo, comienza al interior de esta sociedad, un nuevo debate sobre las fuentes de validación del conocimiento.}%
% %
% \en{At this time, the criterion of personal experience as the foundation of authentic knowledge began to replace the criteria of authority of the Middle Ages.}%
% \es{En esta época, el criterio de experiencia personal como fundamento del saber auténtico comenzó a reemplazar los criterios de autoridad de la Edad Media.}%
% %
% \en{Some understood the concept of experience as ``intellectual evidence'' (rationalists) and others as ``sensible evidence'' (empiricists), two sets of demands that were at first considered incompatible with each other: that of universality and necessity on the one hand, and that of empirical proof on the other.}
% \es{Algunos entendieron el concepto de experiencia como ``evidencia intelectual'' (racionalistas) y otros como ``evidencia sensible'' (empiristas), dos grupos de exigencias que se consideraron al principio incompatibles entre sí: el de universalidad y necesariedad por un lado, y el de acreditación empírica por otro.}%
% %
% \en{However, these new criteria, which allowed the democratization of knowledge within feudal society, were reserved for the exclusive use of its male members.}
% \es{Sin embargo, estos nuevos criterios, que permitían democratizar el conocimiento al interior de la sociedad feudal, estaban reservados para uso exclusivo de sus miembros varones.}%
% %
% % \es{En esta etapa, el sujeto masculino se torna modelo de lo humano y de todo cuanto sea dotado de politicidad, interés general y valor universal, y el espacio de las mujeres se transforma en margen y resto de la política.}%
%
%
% % Parrafo
%
% \en{The argument of moral superiority, developed by Ginés de Sepúlveda four years after the discovery of silver in Potosí, will be used throughout modernity up to the present.}
% \es{El argumento de superioridad moral, que desarrolla Ginés de Sepúlveda cuatro años después del descubrimiento de la plata de Potosí, será utilizado durante toda la modernidad hasta el presente.}%
% \begin{quotation}
%  \en{It will always be just [...] that such [barbarian] peoples submit to the rule of more cultured and humane princes and nations [...] And if they reject such rule it can be imposed on them by means of arms, and such a war will be just as natural law declares [...] In short: it is just, convenient, and in accordance with natural law that men of integrity, intelligence, virtue, and humanity should dominate over all those who do not have these qualities.}%
%  \es{Será siempre justo [...] que tales gentes [bárbaras] se sometan al imperio de príncipes y naciones más cultas y humanas [...] Y si rechazan tal imperio se les puede imponer por medio de las armas, y tal guerra será justa según el derecho natural lo declara [...] En suma: es justo, conveniente y conforme a la ley natural que los varones probos, inteligentes, virtuosos y humanos dominen sobre todos los que no tienen estas cualidades.}%~\cite{GinesdeSepulveda1967p87}.
% \end{quotation}
% %
% \en{This idea continues to be in force even when it is recognized that the universal validity of knowledge would lie in mutual recognition.}
% \es{Esta idea continúa vigente incluso cuando se propone que la validez universal del conocimiento radicaría en el reconocimiento mutuo.}%
% %
% \begin{quotation}
...
% \end{quotation}
% %
% \en{However, Kant himself, who proposes this principle, does not perceive a contradiction when he later states that the American races are incapable of all culture and that the black races can be educated but only as servants and slaves.}%
% \es{Sin embargo, el mismo Kant, quien propone este principio, no percibe una contradicción cuando afirma luego que las razas americanas son incapaces de toda cultura y que las razas negras pueden ser educadas pero sólo como sirvientes y esclavos.}%
% %
% \en{There is no contradiction because the feudal-colonial-modern concept of ``universality'' is born limited.}%
% \es{No hay contradicción porque el concepto feudal-colonial-moderno de ``universalidad'' nace limitado.}%
% %
% \en{Women, foreigners, animals and entire ecological systems participate only as objects of white males, the latter being the only ones endowed with universal value.}%
% \es{Mujeres, extranjeros, animales y sistemas ecológicos completos participan sólo como objetos de los varones blancos, siendo estos últimos los únicos dotados con valor universal.}%
% %
% \en{Colonial-modern society, and its science, repeats the verticalist and punitivist structure of feudal society, now with a global scope.}%
% \es{La sociedad colonial-moderna, y su ciencia, repite la estructura verticalista y punitivista de la sociedad feudal, ahora con un alcance global.}%
% % Todos los conocimientos que la sociedad colonial va incorporando del resto de comunidades del mundo se les borra su verdadero origen histórico y se las enaltecen como surgimientos espontáneos internos.
%
% % Parrafo

\en{Until the early 1800s Western Europe continued to face a trade deficit with China, which was financed since the mid-1700s by the opium drug trade despite China's prohibition.}%
\es{Hasta comienzos del 1800 Europa occidental seguía teniendo déficit comercial con China, que desde mediados de 1700 financiaba mediante el narcotráfico de opio a pesar de la prohibición China.}%
%
\en{The consequences were more severe when a cheaper and more potent opium mixture was developed in 1818.}%
\es{Las consecuencias fueron más graves cuando, después de las independiencias americanas, en 1818, los británicos comienzan a comercializar una mezcla de opio más barata y potente.}%
%
...
\centering
 ...
 \end{figure}
 %
\en{The results were terrible: China lost 1/5 of its population and was mired for a century by foreign invasions.}%
\es{Los resultados fueron terribles: China pierde 1/5 de su población y queda sumida durante un siglo por invasiones extranjeras.}%

% Parrafo

\en{When Western Europe finally establishes itself as the global geopolitical center, after the defeat of China, the era of genocides and massive loss of global cultural diversity begins: the violent occupation of continental Africa and the still autonomous territories of the Americas, among the main ones.}%
\es{Cuando Europa occidental finalmente se establece como el centro geopolítico global, luego de la derrota China, comienza la era de genocidios y masiva pérdida de diversidad cultural global: la ocupación violenta de África continental y de los territorios de América todavía autónomos, entre los principales.}%
%
\en{The criterion of authority globalized during colonial-modernity as a criterion of universality limited to white men opened the door to cultural and ecological arbitrariness.}%
\es{El criterio de autoridad globalizado durante la colonial-modernidad como criterio de universalidad limitado a los varones blancos abrió la puerta a la arbitrariedad cultural y ecológica.}%
%
% \en{It is a new era of scientific and technological advances, but on the other hand it is the era of massive loss of global cultural diversity.}%
% \es{Es una nueva era de avances científicos y tecnológicos, pero por otro es la era de pérdida masiva de diversidad cultural global.}%
%
\en{In a world of weakened communities, the individualistic and instrumental worldview of our time expands: life and people as commodities.}%
\es{En un mundo de comunidades debilitadas, se expande la cosmovisión individualista e instrumental de nuestro tiempo: la naturaleza como cosa, las personas como cosas.}%
%
\en{Despite all the advances, metropolitan science was not able to compensate for the loss of millenarian knowledge caused by colonial-modernity and as a consequence of the massive loss of global cultural diversity, we are currently experiencing a serious and deepening ecological crisis.}%
\es{A pesar de todos los avances, la ciencia metropolitana no fue capaz de compensar la pérdida de conocimientos milenarios provocada por la colonial-modernidad, y como consecuencia de la masiva pérdida de la diversidad cultural global vivimos en la actualidad una grave crisis ecológica que no deja de profundizarse.}%
%
% \begin{figure}[ht!]
% \centering
%  \begin{subfigure}[b]{0.35\textwidth} \centering
%   \includegraphics[width=\linewidth]{static/brasil1986.jpg}
%   %\caption{}
%   \end{subfigure}
%   \begin{subfigure}[b]{0.35\textwidth} \centering
%   \includegraphics[width=\linewidth]{static/brasil2020.jpg}
%   %\caption{}
%   \end{subfigure}
%  \caption{
%  \en{The current massive loss of biodiversity has been preceded by a massive loss of global cultural diversity.}%
%  \es{La masiva pérdida de biodiversidad actual ha estado precedida por una masiva pérdida de la diversidad cultural global.}%
%  }
%  \label{fig:eraDeGenocidios}
%  \end{figure}
%
...
%
\en{Just as selecting a single hypothesis has known negative consequences in probability (overfitting), the imposition of a single type of society is having increasingly obvious ecological consequences for life.}%
\es{Del mismo modo que seleccionar una única hipótesis tiene consecuencias negativas conocidas en probabilidad (overfitting), la imposición de un único tipo de sociedad está teniendo consecuencia ecológicas cada vez más evidentes para la vida.}%
%
\en{Ultimately, in the long term, only the variants capable of reducing fluctuations through diversification and cooperation survive.}%
\es{En definitiva, a largo plazo, solo sobreviven las variantes capaces de reducir las fluctuaciones por diversificación y cooperación.}%

% Parrafo

\en{Thus, the experience accumulated by the most diverse communities in the world has led, independently, to a universal obligation to give and receive, and to the development of technologies of reciprocity that maintain cohesion by re-activating community bonds through (festive or coercive) exchange rites.}%
\es{Así es que la experiencia acumulada por las más diversas comunidades del mundo condujo, de forma independiente, a una obligación universal de dar y de recibir, y al desarrollo de tecnologías de reciprocidad que mantienen la cohesión mediante la re-activación de los vínculos comunitarios mediante ritos (festivos o coercitivos) de intercambios.}%
%
\en{The right to cultural diversity is based on the practical fact that ecologically adapted cultural knowledge evolves only through the experience accumulated by autonomous peoples in the course of their own history.}%
\es{El derecho a la diversidad cultural se sustenta en el hecho práctico de que el conocimiento cultural ecológicamente adaptado evoluciona sólo a través de la experiencia que acumulan los pueblos autónomos en el transcurso de su propia historia.}%
% \es{Los estudios comparados muestran que las instituciones adaptadas ecológicamente evolucionan en comunidades locales que sobreviven a largo plazo gracias al desarrollo de tecnologías de sociabilidad que garantizan el arraigo social y la salud del tejido social.}%
...
\section{La evolución como sistema natural de procesamiento de información}

% Parrafo

\subsection{Naturaleza multiplicativa de los procesos de selección}

En evolución se dice que el crecimiento de un linaje en el tiempo, $\omega(t)$, esta gobernado por una secuencias estocástica de tasas de supervivencia y reproducción $f(\cdot)$ dependientes de un ambiente aleatorio $\Aa$,
%
\begin{equation} \label{eq:modelo_exponencial}
\omega(T) = \prod_t^T f(\Aa(t)) \approx g^T
\end{equation}
%
donde $\Aa(t)$ representa el estado del ambiente en el tiempo $t$ y $g$ representa la tasa de crecimiento caracterísitica cuando $T$ es suficientemente grande.
%
...
\end{equation}
%
Un ejemplo similar fue anlizado por Lewontin y Cohen (1969)~\cite{lewontin1969-randomlyVaryingEnvironment}, en el que la población se reproduce 70\% o sobrevive 50\%.
%
Siguiendo la idea de ese trabajo, y para simplificar el problema, vamos a considerar que el tamaño de la población se describe con un número real positivo y no como un número natural.
%
Debido a que los procesos de selección multiplicativos aparecen más adelante de una vez a lo largo de esta tesis, vamos a remplazar desde ahora la palabra ``población'' por ``recurso'' (sea materia, energía o información).

% Parrafo


Cuando se analiza una proceso de selección evolutivo, el interés está puesto en conocer la ventaja relativa que tienen las diversas estrategias alternativas, $\Ee$.
%%
%Para ponerlo en términos más concretos.
%%
%Supongamos que la estretgia es distribuir los huevos
%
Las diferentes estrategias $\Ee$ las podemos describr con diferentes funciones $f_\Ee(\Aa)$.
%
%Por ejemplo, $f_{0.5}(\Aa) = 1.05$, representa una estrategia que crece $5\%$ sin importar cuál sea el estado del ambiente.
%
Para calcular la ventaja relativa que tiene una estratregia sobre otra, generalmente se calcula la proporción que cada estrategia representa del total, $x_\Ee$.
%
Según el modelo estándar de evolución, conocido como \emph{replicator dynamic} \cite{taylor1978-replicatorDynamic}, el cambio de la proporción de una estrategia en la población depende de su tasa de crecimiento caracterísitica $g_\Ee$,
...
\end{equation}
%
donde el denominador actúa como constante de normalización.
%
¿Cómo se determina la tasa de crecimiento característica, $g$?

% Parrafo

Si nos interesa saber lo que ocurre con infinitas de estas poblaciones evolucionado al mismo tiempo, la estimación correcta se obtiene mediante el valor esperado, $g^t = \langle \omega \rangle_t$.
%
\begin{equation}
\langle \omega \rangle_t = \sum_{\omega \in \Omega_t} \omega \cdot  P(\omega)
\end{equation}
%
Donde $\Omega_t$ es el conjunto de todas las posibles tamaños de la población en el tiempo $t$, y $P(\omega)$ es la probabilidad de que la población tenga el tamaño $\omega$.
%
...
%
Y efectivamente eso es lo que ocurre con el tamaño promedio de las infinitas poblaciones, $\omega(t)$.
%
...
En la figura \ref{fig:ergodicity_cooperation} mostramos la trayectoria de un agente en un grupo cooperador de tamaño 33.
%
\begin{figure}[ht!]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
    ...
    \end{subfigure}
    \caption{
    Los recursos de un individuo en un grupo cooperativo de tamaño 33 (recta verde) se aproxima a la media aritmética (la recta negra debajo de la recta verde).
...
%\subsection{La selección evolutiva de la cooperación}
...
\subsection{Ventaja de diversifiación individual y especialización cooperativa}
...
\en{We have seen, in section ``\nameref{sec:methodology}'', that the individual strategy best adapted to the environment $p=0.71$ was $\Ee^*=0.71$. }%
\es{Hemos visto en la sección ``\nameref{sec:methodology}'' que la estrategia individual mejor adaptada al ambiente $p=0.71$ fue $\Ee^*=0.71$. }%
%
...
%
\en{The red dot represents the conclusion we made in section ``\nameref{sec:methodology}'', that in an environment with $p=0.71$ the best adapted individual strategy is $\Ee^*=0.71$. }%
\es{El punto rojo representa la conclusión que sacamos en la sección ``\nameref{sec:methodology}'', que en un ambiente con $p=0.71$ la estrategia individual mejor adaptada es $\Ee=0.71$. }%
...
\section{La motivación antropológica: la cultura.}
...
%
Muchos mamíferos y otros animales transmiten información por vía social, lo que produce la emergencia de tradiciones simples.
%
En muchas especies de pájaros, por ejemplo, no solo el canto y las rutas migratorias son comportamientos culturales que se transmiten de generación en generación, también pueden desarrollar un conjunto muy amplio de conocimientos específicos adaptados al ambiente que se adquieren primero por experiencia individual y luego son imitados por el resto~\cite{aplin2022-birdsCulture}.

% Parrafo

Si bien otras especies tienen cultura, ninguna a desarrollado un sistema de cultural tan complejo como el ser humamno.
%
A diferencia de otros animales, los humanos acumulamos adaptaciones culturales, modificaciones sucesivas persistentes en el tiempo, produciedo sistemas culturales cada vez más complejos.
%
...
% Parrafo Consecuencias.

El surgimiento de este nuevo sistema de informaci\'on produjo cambios radicales para nuestra especie.
%
Antes de la transici\'on cultural, nuestros ancestros estuvieron en grave peligro de extinci\'on, lo que se evidencia en la baja diversidad del genoma humano, que es significativa menor a la del los chimpances.
%
Luego de la transici\'on cultural nuestra especie fue capaz de ocupar en pocos a\~nos todos los nichos ecol\'ogicos de la tierra como ning\'un otro vertebrado terrestre lo hab\'ia logrado antes~\cite{Boyd2011}.
%
Organizados en sociedades cazadores-recolectores, salimos de África y en pocos años ocupamos Asia, Ocean\'ia y las Am\'ericas (Figura~\ref{fig:agricultura}).
%
...
% cooperacion
¿Por qué la cultura es tan efectiva para la adaptación humana?
%
Como hemos visto en la sección anterior, la naturaleza multiplicativa de los procesos de selección evolutiva produce una ventaja a favor de las variantes que reducen las fluctuaciones.
%
Luego de la transici\'on cultural, cuando el conocimiento dej\'o de ser un recurso individual y pas\'o a ser un recurso cooperativo, dejamos de vernos obligados a reducir las fluctuaciones por diversifiación individual.
%
Mediante la cooperaci\'on de los conocimientos culturales apareci\'o además una ventaja a favor de la especializaci\'on que nos permiti\'o salir del peligro de extinci\'on a ser capaces de ocupar todos los nichos ecol\'ogicos de la tierra organizados en simples sociedades cazadoras-recolectoras.

% Parrafo

% Aprendizaje social
¿Cuales son las características que permite el desarrollo de culturas?
% Crianza cooperativa
¿Por qué otras especies no desarrollaron culturas complejas?
% Evolución cultural
¿Cuales son los mecanismo que producen el desarrollo de culturas complejas?

Revisaremos algunas de las respuestas a estas preguntas en las siguientes subsecciones.

\subsection{Coevolución genético-cultural}

Si bien las tradiciones simples están muy extendidas en las especies animales, y en algunas (ballenas, delfines, primates y aves) las tradiciones son bastante complejas, todas ellas son manifiestamente más simples que la cultura humanas.
%
Ciertamente existen muchas adaptaciones raras, que son particulares a ciertas especies, pero los elementos que son realmente buenos, como el ojo, han evolucionado repetidamente entre las millones de especies animales del mundo.
%
Dado que la cultura ha hecho a los humanos extraordinariamente exitosos, ¿Por qué las otras especies no han adquirido esas capacidades?
%
...
%
Por el contrario, para que evolucionen estas caracterísiticas innatas es necesario la preencia previa de alguna lenguas primitiva que proporcionara una entorno cultural que presionara a favor de la selección de mejores habilidades lingüísticas innatas.
%
...
%
A diferencia de nuestros parientes m\'as cercanos (chimpanc\'es, bonobos, gorilas y orangutanes), en los que la crianza está bajo survisión estricta de la madre, hace aproximadamente 3.5 millones de años, nuestros ancestros comenzaron a desarrollar un tipo de crianza cooperativa.
%
...
%
La capacidades para desarrollar culturas complejas son parte de nuestra biología.
%
Lo que aprendemos, lo que sentimos, cómo pensamos y cómo recordamos están moldeados por la arquitectura de las mentes y los cuerpos humanos formados durante milenios por la acción continua de la evolución orgánica.

\subsection{Evolución cultual y aprendizaje social}
...
Para que emerga una cultural, sea simple o compleja, es necesario que se desarrolle algún tipo de capacidad de aprendizaje social, como la imitación, que permita la transmisión de información entre individuos.
%
De esta forma, una vez que aparece una innovación costosa de aprender por experiencia individual, el resto de los miembros de la sociedad tienen acceso a la solución sin necesidad de redescubrirla cada vez.
%
En las culturas humanas, en las que la acumulación de innovaciones pone a disposición ideas que ningún individuo podría descubrir y tecnologías que ningún individuo podría inventar, se hace inevitable para los individuos adoptarlas con un grado de credulidad, sin comprender su raiz.
%
A pesar de que las personas no sean receptáculos pasivos, la imposibilidad de examinar todas las creencias y valores que a disposición en nuestra cultura, abre la puerta a la difusión de ideas mal adaptadas.
%
Por ello, si bien el desarrollo de la cultura ha sido beneficioso para los seres humanos, la aparición de las capacidades de transmisión social de información pueden también ser contraproducente.
...
% \cite{GonzalezForero2018}

% La comprensi\'on mutua, la imitaci\'on y el lenguaje permitieron la transmisi\'on de conocimientos basados en la experiencia de otros (\emph{aprendizaje social}), dando inicio a un proceso inter-generacional de acumulación de innovaciones que permiti\'o a las poblaciones humanas adaptarse r\'apidamente a los cambiantes contextos ambientales.
% %
% Lo que antes debía ser redescubierto una y otra vez mediante costosa experiencia individual, ahora podía ser transmitido a la siguiente generación.
% %
% La capacidad de adquirir comportamientos basados en la experiencia de otros sin tener que re-construirlos cada vez por prueba y error conduce un proceso de


% \subsection{PONER TITULO}
%
% % Parrafo
%
...
En esta tesis nos proponemos estudiar el efecto que algunos factores sociales tienen sobre el aprendizaje individual.
%
Debido a que la cultura es un fen\'omeno poblacional que emerge como consecuencia del intercambio de informaci\'on entre individuos, el contexto social, la estabilidad o volatilidad de los v\'inculos, la estructura de la red de intercambios o el tama\~no de la poblaci\'on pueden afectar directa o indirectamente los procesos de aprendizaje social y acumulaci\'on cultural de los que dependen los individuos.
%
Hasta ahora, las respuestas con mayor influencia para la antropología se han propuesto a través de modelos matemáticos de evolución cultural.
%
De allí surgieron, por ejemplo, las hipótesis de que la capacidad de acumulación cultural es proporcional al tamaño efectivo de la población, y que estructuras con moderada conexión favorecen la innovación cultural.
%
Si bien los resultados de estos modelos sirven para ganar intuición de procesos complejos, sus resultados son analogías que no están destinadas a realizar predicciones y por lo tanto poseen una débil corroboración empírica.
...
Esta tesis se enmarca en esta nueva forma de interdisciplina, que pone a prueba los métodos computacionales en base al desempeño que tienen sobre datos reales.
%
Las comunidades virtuales ofrecen un marco ideal para estudiar los comportamientos sociales pues permite estudiar poblaciones grandes con un alto grado de detalle.
%
En esta tesis decidimos centrarnos en las comunidades de videojuegos en línea debido a que ellas son un lugar privilegiado para estudiar diversos factores de aprendizaje social.
...
Para responder estas preguntas desde una perspectiva científica, nos obliga a buscar verdades que tengan validez universal.
%
Si bien los métodos de aprendizaje automático funciona muy bien en ciertos contextos, pueden colapsar si los datos lo que el modelo está acostumbrado.
%
Para computar la incertidumbre correctamente, se hace necesario aplicar estrictamente las reglas de la probabilidad.
%
Al evaluar todo el espacio de hipótesis mediante el solo filtro de la sorpresa, única fuente de información, podemos alcanzar ``verdades empíricas'' que no afirmen más de lo que se sabe, sin decir menos.
%
Aunque a lo largo de la tesis no siempre pudimos computar las distribuciones de creencias que garantizan los acuerdos intersubjetivos en contextos de incertidumbre, hicimos un esfuerzo por ajustarnos a este ideal.
...
\chapter{Metodología: enfoque bayesiano y modelo de habilidad.}
...
En la sección \ref{sec:base_empirica_metodo} discutiremos la estructura invariante del dato empírico y el rol que cumplen las hipótesis en la construcción de datos.
%
Allí ... .
%
En la sección \ref{sec:teo_info} revisamos los elementos de la teoría de la información que garantizan una mejor comunicación con la realidad.
%
Allí ... .
%
En la sección \ref{sec:trueskill} mostramos en detalle uno de los modelo bayesianos más utilizados en el industria del video juego.
%
Por sus propiedades y su aceptación general, es el estimador de habilidad que usamos para construir estimaciones de habilidad que sirvan de base empírica para aanlizar los factores de aprendizaje social el siguiente capítulo.
...
A diferencia de las ciencias formales, que validan sus proposiciones mediante teoremas, resultados derivados de aplicar las reglas internas a un sistema axiomático cerrado, las ciencias emp\'iricas deben validar sus proposiciones dentro de sistemas abiertos con incertidumbre.
%
Para alcanzar acuerdos intersubjetivos, universales, en contextos de incertidumbre no es suficiente el sistema lógico-deductivo de la ciencias formales.
%
Existe suficiente consenso par afirmar que la teoría de la probabilidad es el razonamiento común a todas las ciencias empíricas, que actúan bajo incertidumbre.
%
...
La idea de que todo lo que ocurre en la naturaleza tiene una causa previa que lo genere se conoce en la historia de la filosof\'ia como el principio de \emph{raz\'on suficiente}.
%
...
%
Esta idea parece estar justificada en la pregunta su complemento, en la imposibilidad de crear azar, de dadas exactamente las mimas condiciones iniciales, encontrar un ``método efectivo'' que genere a veces un resultado y a veces otro.
%
Antes del desarrollo de las ciencias de la computación existía una intuición respecto de su imposibilidad.
%
La imposibilidad de crear números aleatorios está incluida en la noción misma de ``método efectivo'', que se construye sobre el coonceptp de función.

% Parrafo

Las funciones son la base de los conceptos de causa y efecto, $f(x) = y$, donde $x$ representa las condiciones iniciales, $f$ el modelo causal determinista, e $y$ el efecto.
%
...
%
Sin embargo, vivir en un mundo determinista no nos libera de la incertidumbre.
%
Conocer el mundo con precisi\'on \emph{finita} ha mostrado ser suficiente para introducir una incertidumbre que impide predecir el comportamiento de modelos causales, incluso en el corto plazo.
%
...
%
Supongamos que sabemos que una persona esconder un regalo detr\'as de una caja entre tres.
%
En este casos, ya no es posible definir una funci\'on que dado el estados inicial del universo $s$ devuelva la acci\'on $a$ que tomar\'a el sujeto, $f(s) = a$.
%
