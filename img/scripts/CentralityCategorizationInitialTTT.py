    ...
    for centrality in CENTRALITY_NAMES.keys():
        for summary_type in ['mean', 'median']:
            cents = pickle.load(open(
                'data/initial-ttt-categorization-groups/{}-{}-ttt-{}-50-games-categorization-groups.pickle'.format(
                    summary_type, centrality, ttt), 'rb'))
            ...
            exp_skill_ax.set_ylabel('Habilidad (TrueSkill Through Time)')
            ...
            dir_name = 'resultados/evolucion-ttt/ttt-inicial/{}'.format(summary_type)
...
def main():
    ...
    for player, skills in skill_windows_pp.items():
        flattened = [skill_std_pair for window in skills for skill_std_pair in window]
        if len(flattened) > 0 and 12 <= flattened[0][0] <= 15:
            skill_evolution[player] = flattened
    ...
    for player, skills in skill_windows_pp.items():
        flattened = [skill_std_pair for window in skills for skill_std_pair in window]
        if len(flattened) > 0 and 17 <= flattened[0][0] <= 20:
            skill_evolution[player] = flattened
